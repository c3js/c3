...
"path").attr("class","tick major").style("opacity",1E-6),la=d3.transition(E.exit()).style("opacity",1E-6).remove(),ma=d3.transition(E).style("opacity",1),A,F=J.rangeExtent?J.rangeExtent():La(J.range()),w=w.selectAll(".domain").data([0]),w=(w.enter().append("path").attr("class","domain"),d3.transition(w)),B=J.copy(),Ma=this.__chart__||B;this.__chart__=B;N.append("line");N.append("text");var ua=N.select("line"),va=ma.select("line"),O=E.select("text"),wa=N.select("text"),ba=ma.select("text");U=(B(1)-
