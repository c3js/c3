...
function(a){if(!arguments.length)return a=H.domain(),[a[0],a[1]+1];H.domain(a);return f}}return f}function I(a,b){return e.scale.linear().range([a,b])}function u(a){return"y2"===R(a)?aa:z}function B(a){return"y2"===R(a)?jb:ib}function rc(a,b){var c=(ja?wd():e.svg.axis()).scale(a).orient(b);c.tickFormat(sc());ja?c.categories(Ma).tickCentered(Dd):c.tickOffset=function(){return 0};return c}function R(a){return a in xc?xc[a]:"y"}function sc(){var a=D?Ed:ja?Fd:null;mb&&("function"===typeof mb?a=mb:D&&
(a=function(a){return e.time.format(mb)(a)}));return a}function Z(a){var b=!1;Ob(k.data.targets).forEach(function(c){b||c.data.id!==a.data.id||(b=!0,a=c)});return b?a:null}function ab(){var a=e.svg.arc().outerRadius(La).innerRadius(Gb),b=function(b,d){var f;return d?a(b):(f=Z(b))?a(f):"M 0 0"};b.centroid=a.centroid;return b}function Ja(a){var b=e.svg.arc().outerRadius(Fb*(a?a:1)).innerRadius(Gb);return function(a){return(a=Z(a))?b(a):"M 0 0"}}function zb(a){a=Z(a);var b,c;b="";a&&(b=lb.centroid(a),
a=b[0],b=b[1],c=Math.sqrt(a*a+b*b),b="translate("+0.8*(a/c)*La+","+0.8*(b/c)*La+")");return b}function yd(a){return Gd(a,(a.endAngle-a.startAngle)/(2*Math.PI))}function yc(a,b){var c=n.selectAll(".chart-arc"+ka(a)),d=n.selectAll(".-arc").filter(function(b){return b.data.id!==a});c.selectAll("path").transition().duration(50).attr("d",vc).transition().duration(100).attr("d",wc).each(function(a){Pb(a.data)});b||d.style("opacity",0.3)}function Qb(a){n.selectAll(".chart-arc"+ka(a)).selectAll("path").transition().duration(50).attr("d",
...
a(c);d=m(c[0].x);b=u(b.id)(c[0].value);return h?"M "+b+" "+d:"M "+d+" "+b}}(),xb=function(a,b){var c=a.__max__+1,d=1<arguments.length?b:!0,e=ja?c?1.2*J.tickOffset()/c:0:0.6*((h?s:r)*(d?1:ya(T)/ya(m.domain()))/(gb()-1)),g=fe(e,c,a,!!d),k=ge(!!d),l=he(a,!!d),n=d?B:u;return function(a,b){var c=n(a.id)(0),d=l(a,b)||c,c=[[g(a),d],[g(a),k(a)-(c-d)],[g(a)+e,k(a)-(c-d)],[g(a)+e,d]],d=h?1:0,m=h?0:1;return"M "+c[0][d]+","+c[0][m]+" L"+c[1][d]+","+c[1][m]+" L"+c[2][d]+","+c[2][m]+" L"+c[3][d]+","+c[3][m]+" z"}},
...
k.selected=function(a){a=yb(a);return e.merge(l.selectAll(".-shapes"+a).selectAll(".-shape").filter(function(){return e.select(this).classed(na)}).map(function(a){return a.map(function(a){return a.__data__})}))};k.select=function(a,b,c){la&&l.selectAll(".-shapes").selectAll(".-shape").each(function(d,f){var g="circle"===this.nodeName?dd:me,h="circle"===this.nodeName?Xb:fd;0<=b.indexOf(f)?ma(d)&&(ua||y(a)||0<=a.indexOf(d.id))&&g(e.select(this).classed(na,!0),d,f):v(c)&&c&&h(e.select(this).classed(na,
