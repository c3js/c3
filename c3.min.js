...
"path").attr("class","tick major").style("opacity",1E-6),ma=d3.transition(E.exit()).style("opacity",1E-6).remove(),na=d3.transition(E).style("opacity",1),A,F=J.rangeExtent?J.rangeExtent():La(J.range()),w=w.selectAll(".domain").data([0]),w=(w.enter().append("path").attr("class","domain"),d3.transition(w)),B=J.copy(),Ma=this.__chart__||B;this.__chart__=B;N.append("line");N.append("text");var ua=N.select("line"),va=na.select("line"),O=E.select("text"),wa=N.select("text"),ba=na.select("text");U=(B(1)-
...
".32em").style("text-anchor","end"),O.text(function(c){return n.length<Q||0==c%Math.ceil(n.length/Q)?c<P.length?P[c]:c:""}),w.attr("d","M"+-V+","+F[0]+"H0V"+F[1]+"H"+-V)}if(J.ticks)N.call(A,Ma),na.call(A,B),ma.call(A,B);else{var xa=B.rangeBand()/2,ma=function(c){return B(c)+xa};N.call(A,ma);na.call(A,ma)}})}var J=d3.scale.linear(),p="bottom",G=6,V=6,K=3,c=!1,Q=10,U=0,P=[];n.scale=function(c){if(!arguments.length)return J;J=c;return n};n.orient=function(c){if(!arguments.length)return p;p=c in{top:1,
...
a.length;e++){if(!(a[e]in d))return b;d=d[a[e]]}return d}function Q(){bb=null===yb?+d3.select(ya).style("width").replace("px",""):yb;var a=null===zb?+d3.select(ya).style("height").replace("px",""):zb;oa=0<a?a:320;Ab=20+Bb+ca;Cb=!Db||f||cb?1:50;Na=db?0:40;Eb=oa-Bb-ca;Fb=20+ca;Gb=oa-ca;p=0;G=Cb;V=Ab;K=Na;ta=Eb;n=Fb;J=Na;xb=Gb;La=Na;r=bb-K-G;u=oa-p-V;da=oa-ta-n}function U(){var a,b,d,e;Hb=f?10:0;Ib=f?u:r;eb=f?0:u;fb=f?r:1;a=P(Hb,Ib);b=w(eb,fb);d=w(eb,fb);e=P(0,r);m=k(m)?a.domain(m.domain()):a;y=k(y)?
...
c);return[qc(d,"bar")?0:e-f,g+q]}function N(a){var b;a?(a=m.domain(),b=D.extent()):(a=Jb,b=m.domain());return(a[1]-a[0])/(b[1]-b[0])}function ma(a){for(var b=0;b<a.length;b++)if(!(a[b]in Sa))return!1;return!0}function na(a){for(var b=[],d=0;d<a.length;d++)a[d]in Sa&&b.push(wa(Sa[a[d]]));return b}function A(a){return a in Kb?Kb[a]:"y"}function F(a){return"y2"===A(a)?R:y}function B(a){return"y2"===A(a)?Pa:Oa}function Ma(a){var b=Ca[a.id];a.name=k(b)?b:a.id;return a}function ua(a){var b=a[0],d={},e=
...
a?[].concat(a.classes).join(" "):"")}function Va(a){return m(a.x)}function Da(a){return m(H?ha(a.value):a.value)}function Ob(a){return y(a.value)}function W(a){return m(a.x)}function X(a){return F(a.id)(a.value)}function vc(){var a={},b=0,d,e;Ra(za).forEach(function(c){for(d=0;d<v.length;d++)if(!(0>v[d].indexOf(c.id)))for(e=0;e<v[d].length;e++)if(v[d][e]in a){a[c.id]=a[v[d][e]];break}s(a[c.id])&&(a[c.id]=b++)});a.__max__=b-1;return a}function Pb(a,b,d,e){var c=e?z:m;return function(e){var q=e.id in
...
typeof Ha?Ha:H?Ha(Ua,gb):Ha(0,ba()-1));la&&(x.append("g").attr("clip-path",Ga).attr("class","chart"),x.select(".chart").append("g").attr("class","chart-bars"),x.select(".chart").append("g").attr("class","chart-lines"),x.append("g").attr("clip-path",Ga).attr("class","x brush").call(D).selectAll("rect").attr("height",da),x.append("g").attr("class","x axis").attr("transform",L.subx).call(fa));qa&&Xa(b);dc(b);I({withTransition:!1});if(Jc){if(H&&"string"==typeof ra){ra=ha(ra);for(a=0;a<b[0].values.length&&
...
z.domain():D.extent()),pa&&Fa.x(m));y.domain(E(l.data.targets,"y"));R.domain(E(l.data.targets,"y2"));h.select(".x.axis").transition().duration(f?t:0).call(f?ea:C);h.select(".y.axis").transition().duration(f?0:t).call(f?C:ea);h.select(".y2.axis").transition().call(Qa);Oa.domain(y.domain());Pa.domain(R.domain());la&&null!==d3.event&&"zoom"===d3.event.type&&(D.extent(m.domain()),D.empty()||x.select(".x.brush").call(D));if(Zb){if("year"===Kc)for(a=[],firstYear=Ua.getFullYear(),lastYear=gb.getFullYear(),
...
f?W:X);b.enter().append("circle").attr("class",tc).attr("cx",f?X:W).attr("cy",f?W:X).attr("r",ob);b.exit().remove();v&&la&&(x.select(".x.axis").transition().duration(f?t:0).call(f?ea:fa),n=ga?1.2*fa.tickOffset()/c:0.6*((f?u:r)*N(!0)/(ba()-1)),q=Rb(da,!0),a=Pb(n,c,d,!0),g=Qb(q,d,!1,!0),contextBar=x.selectAll(".-bars").selectAll(".-bar").data(Sb),contextBar.transition().duration(t).attr("x",a).attr("y",g).attr("width",n).attr("height",q),contextBar.enter().append("rect").attr("class",Nb).attr("x",a).attr("y",
...
[d3.time.format(":%S"),function(a){return a.getSeconds()}],[d3.time.format(".%L"),function(a){return a.getMilliseconds()}]]),ha=d3.time.format(Pc).parse,Ab,Fb,Cb,Na,Eb,Gb,r,u,da,bb,oa,Hb,Ib,eb,fb,m,y,R,z,Oa,Pa,C=ga?wb():d3.svg.axis(),ea=d3.svg.axis(),Qa=d3.svg.axis(),fa=ga?wb():d3.svg.axis();Q();U();var L={main:function(){return"translate("+K+","+p+")"},context:function(){return"translate("+J+","+ta+")"},legend:function(){return"translate("+La+","+xb+")"},y2:function(){return"translate("+(f?0:r)+
...
-1).attr("y",-1).attr("width",r+2).attr("height",40);ub.append("clipPath").attr("id","yaxis-clip").append("rect").attr("x",-K+1).attr("y",p-1).attr("width",K).attr("height",u-p+2);var h=Za.append("g").attr("transform",L.main),x=null;la&&(x=Za.append("g").attr("transform",L.context));var sa=null;qa&&(sa=Za.append("g").attr("transform",L.legend));var ja=d3.select(ya).style("position","relative").append("div").style("position","absolute").style("width","30%").style("z-index","10").style("visibility",
"hidden"),D=d3.svg.brush().on("brush",function(){I({withTransition:!1,withY:!1,withSubchart:!1})}),Fa=d3.behavior.zoom().on("zoom",pa?Nc:null),Ua=null,gb=null,Jb;l.focus=function(a){l.defocus();d3.selectAll(Ka(a)).filter(function(a){return ab(a.id)}).classed("focused",!0).transition().duration(100).style("opacity",1)};l.defocus=function(a){d3.selectAll(Ka(a)).filter(function(a){return ab(a.id)}).classed("focused",!1).transition().duration(100).style("opacity",0.3)};l.revert=function(a){d3.selectAll(Ka(a)).filter(function(a){return ab(a.id)}).classed("focused",
!1).transition().duration(100).style("opacity",1)};l.show=function(a){d3.selectAll(Ka(a)).transition().style("opacity",1)};l.hide=function(a){d3.selectAll(Ka(a)).transition().style("opacity",0)};l.load=function(a){s(a.done)&&(a.done=function(){});if("cacheIds"in a&&ma(a.cacheIds))Ia(na(a.cacheIds),a.done);else if("data"in a)Ia(O(data),a.done);else if("url"in a)d3.csv(a.url,function(b,c){Ia(O(c),a.done)});else if("rows"in a)Ia(O(ua(a.rows)),a.done);else if("columns"in a)Ia(O(va(a.columns)),a.done);
...
function(){Q();U();D.x(z);D.empty()||D.extent(m.domain());pa&&Fa.x(m);d3.select("svg").attr("width",bb).attr("height",oa);d3.select("#"+tb).select("rect").attr("width",r).attr("height",u);d3.select("#xaxis-clip").select("rect").attr("width",r+2);d3.select(".zoom-rect").attr("width",r);h.select(".x.axis").attr("transform",L.x);h.select(".y2.axis").attr("transform",L.y2);la&&(x.select(".x.brush").selectAll("rect").attr("height",da),x.select(".x.brush").call(D),x.attr("transform",L.context),x.select(".x.axis").attr("transform",
L.subx));qa&&(sa.attr("transform",L.legend),Xa(l.data.targets,{withTransition:!1}));I({withTransition:!1})};return l}})(window);
