...
typeof e}var zb=K.c3={},e=K.d3;zb.generate=function(R){var pc,qc,xb,rc,p,q,D,M,Z;function g(a,b){for(var c=R,d=0;d<a.length;d++){if(!(a[d]in c))return b;c=c[a[d]]}return c}function xa(){Ab=Ia?Ia:L();ya=S();q=0;D=(h?Bb+vc:0)+(bb?bb:cb?1:40);M=20+(h?0:Bb)+sa;Z=db?db:Cb?eb||h?1:40:1;r=Ab-D-Z;t=ya-q-M;xb=h?q:ya-Bb-sa;rc=h?wc:D;p=20+sa;Db=h?D-wc-vc:r;fb=h?t:ya-xb-p;pc=ya-sa;qc=D;V()}function V(){Eb=t/2;Ja=0.95*Eb;Fb=ja(k.data.targets,"donut")?0.6*Ja:0}function pa(){var a=n.property("offsetLeft");return a?
...
g="M",k,l,m,jd,n,va=[];if(y(d))for(f=0;f<d.length;f++)va[f]={},x(d[f].start)?va[f].start=a[0].x:va[f].start=E?ta(d[f].start):d[f].start,x(d[f].end)?va[f].end=a[a.length-1].x:va[f].end=E?ta(d[f].end):d[f].end;jd=h?function(a){return c(a.value)}:function(a){return b(a.x)};n=h?function(a){return b(a.x)}:function(a){return c(a.value)};d=E?function(a,d,f,e){var g=a.x.getTime();a=d.x-a.x;d=new Date(g+a*(f+e));return"M"+b(new Date(g+a*f))+" "+c(l(f))+" "+b(d)+" "+c(l(f+e))}:function(a,d,f,e){return"M"+b(k(f))+
" "+c(l(f))+" "+b(k(f+e))+" "+c(l(f+e))};for(f=0;f<a.length;f++)if(x(va)||!oe(a[f].x,va))g+=" "+jd(a[f])+" "+n(a[f]);else{k=P(a[f-1].x,a[f].x);l=T(a[f-1].value,a[f].value);e=b(a[f].x)-b(a[f-1].x);m=c(a[f].value)-c(a[f-1].value);e=Math.sqrt(Math.pow(e,2)+Math.pow(m,2));m=2/e;var p=2*m;for(e=m;1>=e;e+=p)g+=d(a[f-1],a[f],e,m)}return g}function cc(a){var b;rb=e.select(za);if(rb.empty())K.alert('No bind element found. Check the selector specified by "bindto" and existance of that element. Default "bindto" is "#chart".');
else{rb.html("");k.data.x={};k.data.targets=Qa(a);xa();sc();m.domain(e.extent(nb(k.data.targets)));A.domain(lb("y"));aa.domain(lb("y2"));W.domain(m.domain());hb.domain(A.domain());ib.domain(aa.domain());$=m.domain();I.scale(W);ea&&C.scale(m);n=e.select(za).append("svg").attr("width",r+D+Z).attr("height",t+q+M).on("mouseenter",te).on("mouseleave",ue);sb=n.append("defs");sb.append("clipPath").attr("id",dc).append("rect").attr("y",q).attr("width",r).attr("height",t-q);sb.append("clipPath").attr("id",
"xaxis-clip").append("rect").attr("x",-1-D).attr("y",-20).attr("width",fa).attr("height",zb);sb.append("clipPath").attr("id","yaxis-clip").append("rect").attr("x",-D+1).attr("y",q-1).attr("width",yd).attr("height",Y);l=n.append("g").attr("transform",J.main);G=Ea?n.append("g").attr("transform",J.context):null;Fa=Ga?n.append("g").attr("transform",J.legend):null;X=e.select(za).style("position","relative").append("div").style("position","absolute").style("z-index","10").style("display","none");l.append("g").attr("class",
...
J.arc).append("text").attr("class","chart-arcs-title").style("text-anchor","middle").text(ye);if(ea)l.insert("rect",ze?null:"g.grid").attr("class","zoom-rect").attr("width",r).attr("height",t).style("opacity",0).style("cursor",h?"ns-resize":"ew-resize").call(C).on("dblclick.zoom",null);null!==tb&&I.extent("function"!==typeof tb?tb:tb(nb()));Ea&&(G.append("g").attr("clip-path",Xa).attr("class","chart"),G.select(".chart").append("g").attr("class","chart-bars"),G.select(".chart").append("g").attr("class",
...
hc)),Zc(d),100>Ub(c,a)?n.select(".event-rect").style("cursor","pointer"):n.select(".event-rect").style("cursor",null))}).on("click",function(){var a,c;F(k.data.targets)||(a=e.mouse(this),c=Vc(k.data.targets,a),100>Ub(c,a)&&l.select(".-circles-"+c.id).select(".-circle-"+c.index).each(function(){pd(this,c,c.index)}))}).call(e.behavior.drag().origin(Object).on("drag",function(){qd(e.mouse(this))}).on("dragstart",function(){rd(e.mouse(this))}).on("dragend",function(){sd()})).call(C).on("dblclick.zoom",
null)}function pd(a,b,c){var d=e.select(a),f=d.classed(oa),g=!1,h;"circle"===a.nodeName?(g=dd(a,1.5*Wa),h=gd):"rect"===a.nodeName&&(g=ed(a),h=id);if(wa||g)ma&&na(b)&&(d.classed(oa,!f),h(!f,d,b,c)),Ee(b,d)}function qd(a){var b,c,d,f,g,h,m;F(k.data.targets)||!ma||ea&&!C.altDomain||(b=jc[0],c=jc[1],d=a[0],a=a[1],f=Math.min(b,d),g=Math.max(b,d),h=wa?q:Math.min(c,a),m=wa?t:Math.max(c,a),l.select(".dragarea").attr("x",f).attr("y",h).attr("width",g-f).attr("height",m-h),l.selectAll(".-shapes").selectAll(".-shape").filter(function(a){return na(a)}).each(function(a,
...
t)):(Tb?(g=function(a,b){var c=Jc(b),d=b<Ka()-1?k.data.targets[0].values[b+1].x:void 0;return(m(d?d:a.x+50)-m(c?c:a.x-50))/2},d=function(a,b){var c=Jc(b);return(m(a.x)+m(c?c:a.x-50))/2}):(g=ga(),d=function(a){return m(a.x)-g/2}),f=ce(),l.select(".event-rects").datum(f?f.values:[]),f=l.select(".event-rects").selectAll(".event-rect").data(function(a){return a}),Ce(f.enter()),f.attr("class",Pc).attr("x",h?0:d).attr("y",h?d:0).attr("width",h?r:g).attr("height",h?g:t),f.exit().remove());d=l.select(".regions").selectAll("rect.region").data(Q);
