...
O.c3={},e=O.d3;zb.generate=function(ea){var oc,pc,Ga,Ha,p,q,C,M,$;function g(a,b){for(var c=ea,d=0;d<a.length;d++){if(!(a[d]in c))return b;c=c[a[d]]}return c}function ua(){Ab=Ja?Ja:Q();va=fa();q=0;C=(h?Bb+sc:0)+(bb?bb:cb?1:40);M=20+(h?0:Bb)+oa;$=db?db:Cb?eb||h?1:40:1;r=Ab-C-$;s=va-q-M;Ga=h?q:va-Bb-oa;Ha=h?tc:C;p=20+oa;Db=h?C-tc-sc:r;fb=h?s:va-Ga-p;oc=va-oa;pc=C;S()}function S(){Eb=s/2;Ka=0.95*Eb;Fb=ia(k.data.targets,"donut")?0.6*Ka:0}function fa(){var a=Gb?Gb:+e.select(wa).style("height").replace("px",
...
function(a){return 0===a%1?hb.tickOffset():0});ib=I(Lb,Mb);jb=I(Lb,Mb);J=qc(m,yd);ya=e.svg.axis().scale(z).orient(zd);kb=e.svg.axis().scale(aa).orient(Ad);hb=qc(U,Bd);ga()}function ga(){lb=ab();uc=Ia();vc=Ia(0.98)}function ha(a,b,c,d){var f=(D?e.time.scale():e.scale.linear()).range([a,b]);f.orgDomain=function(){return f.domain()};t(c)&&f.domain(c);y(d)&&(d=function(){return 0});if(ja){var H=f,u,f=function(a){return H(a)+d(a)};for(u in H)f[u]=H[u];f.orgDomain=function(){return H.domain()};f.domain=
function(a){if(!arguments.length)return a=H.domain(),[a[0],a[1]+1];H.domain(a);return f}}return f}function I(a,b){return e.scale.linear().range([a,b])}function v(a){return"y2"===R(a)?aa:z}function B(a){return"y2"===R(a)?jb:ib}function qc(a,b){var c=(ja?vd():e.svg.axis()).scale(a).orient(b);c.tickFormat(rc());ja?c.categories(La).tickCentered(Cd):c.tickOffset=function(){return 0};return c}function R(a){return a in wc?wc[a]:"y"}function rc(){var a=D?Dd:ja?Ed:function(a){return a};mb&&(a="function"===
...
lb.centroid(a),a=b[0],b=b[1],c=Math.sqrt(a*a+b*b),b="translate("+0.8*(a/c)*Ka+","+0.8*(b/c)*Ka+")");return b}function xd(a){return Fd(a,(a.endAngle-a.startAngle)/(2*Math.PI))}function xc(a,b){var c=n.selectAll(".chart-arc.target"+(a?"-"+a:"")),d=n.selectAll(".-arc").filter(function(b){return b.data.id!==a});c.selectAll("path").transition().duration(50).attr("d",uc).transition().duration(100).attr("d",vc).each(function(a){Ob(a.data)});b||d.style("opacity",0.3)}function Pb(a){n.selectAll(".chart-arc.target"+
(a?"-"+a:"")).selectAll("path").transition().duration(50).attr("d",lb);n.selectAll(".-arc").style("opacity",1)}function Gd(a){var b=yc(a),c,d,f,H;if(0<w.length)for(H=zc(a),a=0;a<w.length;a++)for(d=w[a][0],H&&b[d]&&b[d].forEach(function(a,c){b[d][c]=0>a?a:0}),c=1;c<w[a].length;c++)f=w[a][c],b[f]&&b[f].forEach(function(a,c){R(f)!==R(d)||!b[d]||H&&0<+a||(b[d][c]+=+a)});return e.min(Object.keys(b).map(function(a){return e.min(b[a])}))}function Hd(a){var b=yc(a),c,d,f,H;if(0<w.length)for(H=Id(a),a=0;a<
...
g=Ma.bottom?Ma.bottom:f);"y2"===a&&Na&&(e=Na.top?Na.top:f,g=Na.bottom?Na.bottom:f);ia(b,"bar")&&!zc(b)&&(g=c);return[c-g,d+e]}function Pd(a){return e.min(a,function(a){return e.min(a.values,function(a){return a.x})})}function Qd(a){return e.max(a,function(a){return e.max(a.values,function(a){return a.x})})}function pb(a){var b=[Pd(a),Qd(a)];a=b[0];var b=b[1],c=ja?0:0.01*Math.abs(a-b);a=D?new Date(a.getTime()-c):a-c;b=D?new Date(b.getTime()+c):b+c;return[a,b]}function xa(a){return a[1]-a[0]}function Rd(a){for(var b=
0;b<a.length;b++)if(!(a[b]in qb))return!1;return!0}function Sd(a){for(var b=[],c=0;c<a.length;c++)a[c]in qb&&b.push(Ac(qb[a[c]]));return b}function Qb(a){return"start"in a?m(D?pa(a.start):a.start):0}function Bc(a){var b=Qb(a);a=("end"in a?m(D?pa(a.end):a.end):r)-b;return 0>a?0:a}function Cc(a){return qa&&a===qa||ba&&Td(ba,a)}function Ud(a){return!Cc(a)}function Vd(a){Object.keys(a).forEach(function(b){ba[b]=a[b]})}function rb(a){var b;a&&(b=Oa[a.id],a.name=b?b:a.id);return a}function Dc(a){var b=
a[0],c={},d=[],f,e;for(f=1;f<a.length;f++){c={};for(e=0;e<a[f].length;e++)c[b[e]]=a[f][e];d.push(c)}return d}function Ec(a){var b=[],c,d,f;for(c=0;c<a.length;c++)for(f=a[c][0],d=1;d<a[c].length;d++)y(b[d-1])&&(b[d-1]={}),b[d-1][f]=a[c][d];return b}function Pa(a){var b=e.keys(a[0]).filter(Ud),c=e.keys(a[0]).filter(Cc);if(D&&0===c.length)return O.alert('data.x or data.xs must be specified when axis.x.type == "timeseries"'),[];Rb&&b.forEach(function(b){var f=qa?qa:ba?ba[b]:null;0<=c.indexOf(f)?k.data.x[b]=
...
b++})});Fc&&ca(Qa(b).filter(function(a){return!(a in A)}),Fc);b.forEach(function(a){qb[a.id_org]=Ac(a)});return b}function Ac(a){return{id:a.id,id_org:a.id_org,values:a.values.map(function(a){return{x:a.x,value:a.value,id:a.id}})}}function Gc(a){return 0<a&&k.data.targets[0].values[a-1]?k.data.targets[0].values[a-1].x:void 0}function gb(){return e.max(k.data.targets,function(a){return a.values.length})}function Xd(){var a=k.data.targets.length,b=0,c;1<a?k.data.targets.forEach(function(a){a.values.length>
b&&(c=a,b=a.values.length)}):c=a?k.data.targets[0]:null;return c}function Qa(a){a=y(a)?k.data.targets:a;return a.map(function(a){return a.id})}function Hc(a){var b=Qa(),c;for(c=0;c<b.length;c++)if(b[c]===a)return!0;return!1}function ob(a){return t(a)?k.data.targets.filter(a):k.data.targets}function yc(a){var b={};a.forEach(function(a){b[a.id]=[];a.values.forEach(function(d){b[a.id].push(d.value)})});return b}function Ic(a,b){var c=Object.keys(a),d,f,e;for(d=0;d<c.length;d++)for(e=a[c[d]].values,f=
0;f<e.length;f++)if(b(e[f].value))return!0;return!1}function zc(a){return Ic(a,function(a){return 0>a})}function Id(a){return Ic(a,function(a){return 0<a})}function Ed(a){return a<La.length?La[a]:a}function Ra(a){return"-shapes -shapes-"+a.id}function Jc(a){return Ra(a)+" -line -line-"+a.id}function Yd(a){return Ra(a)+" -circles -circles-"+a.id}function Kc(a){return Ra(a)+" -bars -bars-"+a.id}function Zd(a){return Ra(a.data)+" -arc -arc-"+a.data.id}function $d(a){return Ra(a)+" -area -area-"+a.id}
function ae(a,b){return"-shape -shape-"+b+" -circle -circle-"+b}function Lc(a,b){return"-shape -shape-"+b+" -bar -bar-"+b}function be(a,b){return"region region-"+b+" "+("classes"in a?[].concat(a.classes).join(" "):"")}function Mc(a,b){return"event-rect event-rect-"+b}function ce(a){return a.value||0===a.value?Nc(a)?0.5:1:0}function za(a){return m(a.x)}function Sa(a){return m(D?pa(a.value):a.value)}function Ta(a){return z(a.value)}function Oc(a){return U(a.x)}function Pc(a,b){var c,d=a[b].x,f=[];for(c=
b-1;0<=c&&d===a[c].x;c--)f.push(a[c]);for(c=b;c<a.length&&d===a[c].x;c++)f.push(a[c]);return f}function Qc(a,b,c,d){c=c?c:0;d=d?d:a.length-1;var f=Math.floor((d-c)/2)+c;0<m(a[f].x)-b[0]?d=f:c=f;return 1===d-c?(f=[],a[c].x&&(f=f.concat(Pc(a,c))),a[d].x&&(f=f.concat(Pc(a,d))),Rc(f,b)):Qc(a,b,c,d)}function Sc(a,b){var c;c=a.map(function(a){return Qc(a.values,b)});return Rc(c,b)}function Rc(a,b){var c,d;a.forEach(function(a){var e=Sb(a,b);if(e<c||!c)c=e,d=a});return d}function Tc(a,b){var c,d,f,e,g;Uc&&
0!==a.filter(function(a){return a&&(a.value||0===a.value)}).length&&(V.html(Vc(a)).style("visibility","hidden").style("display","block"),c=V.property("offsetWidth"),d=V.property("offsetHeight"),h?e=b[0]:(f=n.property("offsetLeft"),e=f+(bb?bb:cb?1:40)+m(a[0].x)+20,g=e+c,f=f+(Ja?Ja:Q())-(db?db:Cb?eb||h?1:40:1),g>f&&(e-=c+30)),c=b[1]+15+d<fa()?b[1]+15:b[1]-d,V.style("top",c+"px").style("left",e+"px").style("visibility","visible"))}function Wc(a){Uc&&(ia(k.data.targets,"scatter")||E(k.data.targets)||
l.selectAll("line.xgrid-focus").style("visibility","visible").data([a]).attr(h?"y1":"x1",za).attr(h?"y2":"x2",za))}function Xc(){l.select("line.xgrid-focus").style("visibility","hidden")}function Aa(a){return a.x||0===a.x?m(a.x):null}function Ba(a){return v(a.id)(a.value)}function de(){var a={},b=0,c,d;ob(sb).forEach(function(f){for(c=0;c<w.length;c++)if(!(0>w[c].indexOf(f.id)))for(d=0;d<w[c].length;d++)if(w[c][d]in a){a[f.id]=a[w[c][d]];break}y(a[f.id])&&(a[f.id]=b++)});a.__max__=b-1;return a}function ee(a,
...
a,c)}))}).call(e.behavior.drag().origin(Object).on("drag",function(){od(e.mouse(this))}).on("dragstart",function(){pd(e.mouse(this))}).on("dragend",function(){qd()})).call(F).on("dblclick.zoom",null)}function Ae(a){a.append("rect").attr("x",0).attr("y",0).attr("width",r).attr("height",s).attr("class","event-rect").on("mouseout",function(){E(k.data.targets)||(Xc(),V.style("display","none"),Zb())}).on("mousemove",function(){var a,c,d;Xa||E(k.data.targets)||(a=e.mouse(this),c=Sc(k.data.targets,a),d=
[rb(c)],Tc(d,a),fc&&(Zb(),Yb(c.index,c.id).classed(da,!0).attr("r",gc)),Wc(d[0]),100>Sb(c,a)?n.select(".event-rect").style("cursor","pointer"):n.select(".event-rect").style("cursor",null))}).on("click",function(){var a,c;E(k.data.targets)||(a=e.mouse(this),c=Sc(k.data.targets,a),100>Sb(c,a)&&l.select(".-circles-"+c.id).select(".-circle-"+c.index).each(function(){nd(this,c,c.index)}))}).call(e.behavior.drag().origin(Object).on("drag",function(){od(e.mouse(this))}).on("dragstart",function(){pd(e.mouse(this))}).on("dragend",
function(){qd()})).call(F).on("dblclick.zoom",null)}function nd(a,b,c){var d=e.select(a),f=d.classed(ma),g=!1,h;"circle"===a.nodeName?(g=ad(a,1.5*Va),h=dd):"rect"===a.nodeName&&(g=bd(a),h=fd);if(ta||g)ka&&la(b)&&(d.classed(ma,!f),h(!f,d,b,c)),Be(b,d)}function od(a){var b,c,d,f,g,h,m;E(k.data.targets)||!ka||sa&&!F.altDomain||(b=ic[0],c=ic[1],d=a[0],a=a[1],f=Math.min(b,d),g=Math.max(b,d),h=ta?q:Math.min(c,a),m=ta?s:Math.max(c,a),l.select(".dragarea").attr("x",f).attr("y",h).attr("width",g-f).attr("height",
m-h),l.selectAll(".-shapes").selectAll(".-shape").filter(function(a){return la(a)}).each(function(a,b){var c=e.select(this),d=c.classed(ma),k=c.classed(jc),l,n,p,q;l=!1;"circle"===this.nodeName?(l=1*c.attr("cx"),n=1*c.attr("cy"),q=dd,l=f<l&&l<g&&h<n&&n<m):"rect"===this.nodeName&&(l=1*c.attr("x"),n=1*c.attr("y"),p=1*c.attr("width"),q=fd,l=f<l+p&&l<g&&n<m);l^k&&(c.classed(jc,!k),c.classed(ma,!d),q(!d,c,a,b))}))}function pd(a){!E(k.data.targets)&&ka&&(ic=a,l.select(".chart").append("rect").attr("class",
"dragarea").style("opacity",0.1),Xa=!0)}function qd(){!E(k.data.targets)&&ka&&(l.select(".dragarea").transition().duration(100).style("opacity",0).remove(),l.selectAll(".-shape").classed(jc,!1),Xa=!1)}function x(a){var b,c,d,f;d=de();var g,u,n,p=E(k.data.targets);a=t(a)?a:{};c=t(a.withY)?a.withY:!0;f=t(a.withSubchart)?a.withSubchart:!0;u=t(a.withTransition)?a.withTransition:!0;n=t(a.withTransform)?a.withTransform:!1;b=t(a.withUpdateXDomain)?a.withUpdateXDomain:!1;a=t(a.withUpdateOrgXDomain)?a.withUpdateOrgXDomain:
...
h?function(a){return m(a)-J.tickOffset()}:q).attr("y2",h?function(a){return m(a)-J.tickOffset()}:s).style("opacity",function(){return+e.select(this).attr(h?"y1":"x1")===(h?s:0)?0:1});b.exit().remove()}dc&&(b=l.selectAll(".xgrid-lines"),b.selectAll("line").transition().duration(u).attr("x1",h?0:Sa).attr("x2",h?r:Sa).attr("y1",h?Sa:q).attr("y2",h?Sa:s),b.selectAll("text").attr("x",h?r:0).attr("y",Sa));c&&jd&&(b=l.select(".ygrids").selectAll(".ygrid").data(z.ticks(10)),b.enter().append("line").attr("class",
"ygrid"),b.attr("x1",h?z:0).attr("x2",h?z:r).attr("y1",h?0:z).attr("y2",h?s:z),b.exit().remove());c&&ec&&(c=l.select(".ygrid-lines"),c.selectAll("line").transition().duration(u).attr("x1",h?Ta:0).attr("x2",h?Ta:r).attr("y1",h?0:Ta).attr("y2",h?s:Ta),c.selectAll("text").attr("x",h?0:r).attr("y",Ta));c=l.selectAll(".-bars").selectAll(".-bar").data($c);c.enter().append("path").attr("d",xb(d,!1)).style("stroke","none").style("opacity",0).style("fill",function(a){return N(a.id)}).attr("class",Lc);c.style("opacity",
...
ce).attr("cx",h?Ba:Aa).attr("cy",h?Aa:Ba);c.exit().remove();l.selectAll(".chart-arc").select(".-arc").attr("transform",n?"scale(0)":"").style("opacity",function(a){return a===this._current?0:1}).transition().duration(u).attrTween("d",function(a){a=Z(a);if(!a)return function(){return"M 0 0"};var b=e.interpolate(this._current,a);this._current=b(0);return function(a){a=b(a);return Zc(a.data)||Ob(a.data)?lb(a,!0):"M 0 0"}}).attr("transform",n?"scale(1)":"").style("opacity",1);l.selectAll(".chart-arc").select("text").attr("transform",
yb).style("opacity",0).transition().duration(u).text(xd).style("opacity",function(a){return Zc(a.data)||Ob(a.data)?1:0});l.select(".chart-arcs-title").style("opacity",ia(k.data.targets,"donut")?1:0);Ca&&(null!==e.event&&"zoom"===e.event.type&&K.extent(m.orgDomain()).update(),f&&(G.select(".x.axis").style("opacity",p?0:1).transition().duration(h?u:0).call(hb),K.empty()||K.extent(m.orgDomain()).update(),f=G.selectAll(".-bars").selectAll(".-bar").data($c),f.enter().append("path").attr("d",xb(d)).style("stroke",
"none").style("fill",function(a){return N(a.id)}).attr("class",Lc),f.style("opacity",Ua).transition().duration(u).attr("d",xb(d)).style("opacity",1),f.exit().transition().duration(u).style("opacity",0).remove(),G.selectAll(".-line").style("opacity",Ua).transition().duration(u).attr("d",Ge).style("opacity",1)));l.selectAll(".selected-circles").filter(function(a){return sb(a)}).selectAll("circle").remove();l.selectAll(".selected-circle").transition().duration(u).attr("cx",h?Ba:Aa).attr("cy",h?Aa:Ba);
ba?(f=l.select(".event-rects").selectAll(".event-rect").data([0]),Ae(f.enter()),f.attr("x",0).attr("y",0).attr("width",r).attr("height",s)):(Rb?(g=function(a,b){var c=Gc(b),d=b<gb()-1?k.data.targets[0].values[b+1].x:void 0;return(m(d?d:a.x+50)-m(c?c:a.x-50))/2},d=function(a,b){var c=Gc(b);return(m(a.x)+m(c?c:a.x-50))/2}):(g=X(),d=function(a){return m(a.x)-g/2}),f=Xd(),l.select(".event-rects").datum(f?f.values:[]),f=l.select(".event-rects").selectAll(".event-rect").data(function(a){return a}),ze(f.enter()),
f.attr("class",Mc).attr("x",h?0:d).attr("y",h?d:0).attr("width",h?r:g).attr("height",h?g:s),f.exit().remove());d=l.select(".regions").selectAll("rect.region").data(P);d.enter().append("rect");d.attr("class",be).attr("x",h?0:Qb).attr("y",h?Qb:q).attr("width",h?r:Bc).attr("height",h?Bc:s).style("fill-opacity",function(a){return t(a.opacity)?a.opacity:0.1});d.exit().transition().duration(u).style("fill-opacity",0).remove();Qa().forEach(function(a){Tb[a]=!0})}function He(){"mousemove"===e.event.sourceEvent.type&&
F.altDomain?(m.domain(F.altDomain),F.scale(m).updateScaleExtent()):(ja&&m.orgDomain()[0]===T[0]&&m.domain([T[0]-1E-10,m.orgDomain()[1]]),x({withTransition:!1,withY:!1,withSubchart:!1}),"mousemove"===e.event.sourceEvent.type&&(hc=!0))}function ye(){function a(){b.forEach(function(a){a()})}var b=[];a.add=function(a){b.push(a)};return a}function md(){ua();Y();K.scale(U);sa&&F.scale(m);n.attr("width",Ab).attr("height",va);n.select("#"+cc).select("rect").attr("width",r).attr("height",s);n.select("#xaxis-clip").select("rect").attr("width",
...
function(a,b){return e+Za*b+14})}function $a(a){return t(a)?".target-"+a:".target"}function mc(a){return Hc(a.id)}function nc(a){return"data"in a&&Hc(a.data.id)}var k={data:{}},qb={},da="_expanded_",ma="_selected_",jc="_included_",wa=g(["bindto"],"#chart"),Ja=g(["size","width"],null),Gb=g(["size","height"],null),bb=g(["padding","left"],null),db=g(["padding","right"],null),sa=g(["zoom","enabled"],!1),sd=g(["zoom","extent"],null),we=g(["zoom","privileged"],!1),qe=g(["onenter"],function(){}),re=g(["onleave"],
function(){}),Ce=g(["transition","duration"],350);if(!("data"in ea))throw Error("data is required in config");var qa=g(["data","x"],null),ba=g(["data","xs"],null),Vb=g(["data","x_format"],"%Y-%m-%d"),Wd=g(["data","id_converter"],function(a){return a}),Oa=g(["data","names"],{}),w=g(["data","groups"],[]),wc=g(["data","axes"],{}),Fc=g(["data","type"],null),A=g(["data","types"],{}),td=g(["data","regions"],{}),Ke=g(["data","colors"],{}),ka=g(["data","selection","enabled"],!1),ta=g(["data","selection",
...
a.interpolate("cardinal"):a.interpolate("linear"),td[b.id]?me(c,m,v(b.id),td[b.id]):a(c);d=m(c[0].x);b=v(b.id)(c[0].value);return h?"M "+b+" "+d:"M "+d+" "+b}}(),Fe=function(){var a;a=h?e.svg.area().x0(function(a){return v(a.id)(0)}).x1(function(a){return v(a.id)(a.value)}).y(za):e.svg.area().x(za).y0(function(a){return v(a.id)(0)}).y1(function(a){return v(a.id)(a.value)});return function(b){var c=Xb(b.values),d;if(ia([b],"area")||ia([b],"area-spline"))return Yc(b)?a.interpolate("cardinal"):a.interpolate("linear"),
...
b.classes.indexOf(a)})});return P};k.data.get=function(a){a=k.data.getAsTarget(a);return t(a)?a.values.map(function(a){return a.value}):void 0};k.data.getAsTarget=function(a){var b=ob(function(b){return b.id===a});return 0<b.length?b[0]:void 0};k.resize=function(a){Ja=a?a.width:null;Gb=a?a.height:null;md()};k.destroy=function(){k.data.targets=void 0;k.data.x={};tb.html("");O.onresize=null};if("url"in ea.data)e.csv(ea.data.url,function(a,b){ac(b)});else if("rows"in ea.data)ac(Dc(ea.data.rows));else if("columns"in
ea.data)ac(Ec(ea.data.columns));else throw Error("url or rows or columns is required.");return k}})(window);
