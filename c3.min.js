...
oa=t.select("line"),xa=P.select("text"),ha=Q.select("text"),eb=t.select("text");ga=(A(1)-A(0))/2;P=Na?0:ga;switch(pa){case "bottom":H=y;X.attr("y2",v);ha.attr("y",Math.max(v,0)+qa);oa.attr("x1",P).attr("x2",P).attr("y2",v);eb.attr("x",0).attr("y",Math.max(v,0)+qa);xa.attr("dy",".71em").style("text-anchor","middle");xa.text(function(d){return La(Ka,d)?Ma(d):""});n.attr("d","M"+L[0]+","+d+"V0H"+L[1]+"V"+d);break;case "left":H=w,X.attr("x2",-v),ha.attr("x",-(Math.max(v,0)+qa)),oa.attr("x2",-v).attr("y2",
0),eb.attr("x",-(Math.max(v,0)+qa)).attr("y",ga),xa.attr("dy",".32em").style("text-anchor","end"),xa.text(function(d){return La(Ka,d)?Ma(d):""}),n.attr("d","M"+-d+","+L[0]+"H0V"+L[1]+"H"+-d)}if(M.ticks)Q.call(H,Y),t.call(H,A),R.call(H,A);else{var Cb=A.rangeBand()/2,R=function(d){return A(d)+Cb};Q.call(H,R);t.call(H,R)}})}var M=e.scale.linear(),pa="bottom",v=6,d=6,qa=3,Na=!1,L=10,ga=0,Y=null,ha=!0,X=[];n.scale=function(d){if(!arguments.length)return M;M=d;return n};n.orient=function(d){if(!arguments.length)return pa;
...
typeof e}function y(e){return"undefined"!==typeof e}var Db=N.c3={},e=N.d3;Db.generate=function(fa){var tc,zb,Ab,La,Ma,n,M,pa,v;function d(a,b){for(var c=fa,g=0;g<a.length;g++){if(!(a[g]in c))return b;c=c[a[g]]}return c}function qa(){Oa=Pa?Pa:Y();ra=L();sa=ia?x?ra:40:0;Qa=ia?x?150:Oa:0;n=h&&fb?20:0;M=ga();pa=20+(h?0:Eb)+(x?0:sa);v=(h?Eb+xc:0)+(B(f.data.targets)?0:gb?gb:hb?1:40);q=Oa-v-M;r=ra-n-pa;Ab=h?n:ra-Eb-(x?0:sa);La=20+(x?0:sa);Ma=h?yc:v;Fb=h?v-yc-xc:q;ib=h?r:ra-Ab-La;tc=x?n:ra-sa;zb=x?Oa-Qa:
...
D}function H(a){return"y2"===A(a)?lb:kb}function uc(a,b,c,g){a=(ja?Fd():e.svg.axis()).scale(a).orient(b);a.tickFormat(c).ticks(g);ja?a.tickCentered(Pd):a.tickOffset=function(){return(h?r:q)*(za($)/za(m.domain()))/Aa()/2};ja&&a.categories(Sa);return a}function A(a){return a in Dc?Dc[a]:"y"}function Bb(){var a=C?Qd:ja?Rd:null;nb&&("function"===typeof nb?a=nb:C&&(a=function(a){return e.time.format(nb)(a)}));return a}function Hd(){var a=Aa();return Sd&&a>Ec?Ec:a}function oa(a){var b=!1;Sb(f.data.targets).forEach(function(c){b||
c.data.id!==a.data.id||(b=!0,a=c)});return b?a:null}function xa(){var a=e.svg.arc().outerRadius(Ra).innerRadius(Gb),b=function(b,g){var k;return g?a(b):(k=oa(b))?a(k):"M 0 0"};b.centroid=a.centroid;return b}function vc(a){var b=e.svg.arc().outerRadius(jb*(a?a:1)).innerRadius(Gb);return function(a){return(a=oa(a))?b(a):"M 0 0"}}function eb(a){a=oa(a);var b,c;b="";a&&(b=mb.centroid(a),a=b[0],b=b[1],c=Math.sqrt(a*a+b*b),b="translate("+0.8*(a/c)*Ra+","+0.8*(b/c)*Ra+")");return b}function Cb(a){return Td(a,
...
function(a){return e.min(a.values,function(a){return a.x})})}function de(a){return e.max(a,function(a){return e.max(a.values,function(a){return a.x})})}function qb(a){var b=[ce(a),de(a)],c=b[0],g=b[1],b=Math.abs(b[0]-b[1]);a=ja?0:Z(a,"bar")?b/(Aa()-1)/2:0.01*b;c=C?new Date(c.getTime()-a):c-a;g=C?new Date(g.getTime()+a):g+a;return[c,g]}function za(a){return a[1]-a[0]}function ee(a){for(var b=0;b<a.length;b++)if(!(a[b]in rb))return!1;return!0}function fe(a){for(var b=[],c=0;c<a.length;c++)a[c]in rb&&
...
-1).attr("y",-20).attr("width",ha).attr("height",X);vb.append("clipPath").attr("id","yaxis-clip").append("rect").attr("x",-v+1).attr("width",Ka).attr("height",Db);l=p.append("g").attr("transform",K.main);G=Ha?p.append("g").attr("transform",K.context):null;Ia=ia?p.append("g").attr("transform",K.legend):null;U=e.select(ya).style("position","relative").append("div").style("position","absolute").style("z-index","10").style("display","none");l.append("g").attr("class","x axis").attr("clip-path",h?"":"url("+
...
G.append("g").attr("clip-path",bb).attr("class","x brush").call(J).selectAll("rect").attr(h?"width":"height",h?Fb:ib),G.append("g").attr("class","x axis").attr("transform",K.subx).attr("clip-path",h?"url("+document.URL+"#yaxis-clip)":""));ia&&xb(f.data.targets);ud(f.data.targets);V({withTransform:!0,withUpdateXDomain:!0});if(Pe){if(C&&"string"===typeof Ja){Ja=ta(Ja);for(a=0;a<f.data.targets[0].values.length&&0!==f.data.targets[0].values[a].x-Ja;a++);Ja=a}U.html(cd(f.data.targets.map(function(a){return sb(a.values[Ja])}),
Bb(),Yb,I));U.style("top",vd.top).style("left",vd.left).style("display","block")}null==N.onresize&&(N.onresize=Qe());N.onresize.add&&N.onresize.add(W)}}function Re(a){a.append("rect").attr("class",Tc).style("cursor",la&&wa?"pointer":null).on("mouseover",function(a,c){if(!cb&&!B(f.data.targets)){var d=f.data.targets.map(function(a){return sb(a.values[c])}),k,e;if(0<Object.keys(Va).length){e=[];for(var h in Va)for(k=0;k<d.length;k++)if(d[k].id===h){e.push(d[k]);d.shift(k);break}d=e.concat(d)}jc&&cc(c,
void 0).classed(da,!0).attr("r",kc);l.selectAll(".-bar"+(F(c)?"-"+c:"")).classed(da,!1);dd(d)}}).on("mouseout",function(a,c){B(f.data.targets)||(ed(),U.style("display","none"),dc(c),l.selectAll(".-bar"+(F(void 0)?"-undefined":"")).classed(da,!1))}).on("mousemove",function(a,c){var d;cb||B(f.data.targets)||(d=f.data.targets.map(function(a){return sb(a.values[c])}),ad(d,e.mouse(this)),la&&(wa||l.selectAll(".-shape-"+c).filter(function(a){return ma(a)}).each(function(){var a=e.select(this).classed(da,
...
function(){zd()})).call(E).on("dblclick.zoom",null)}function wd(a,b,c){var d=e.select(a),k=d.classed(na),f=!1,h;"circle"===a.nodeName?(f=id(a,1.5*ab),h=ld):"rect"===a.nodeName&&(f=jd(a),h=nd);if(wa||f)la&&ma(b)&&(d.classed(na,!k),h(!k,d,b,c)),Te(b,d)}function xd(a){var b,c,d,k,h,m,p;B(f.data.targets)||!la||ea&&!E.altDomain||(b=mc[0],c=mc[1],d=a[0],a=a[1],k=Math.min(b,d),h=Math.max(b,d),m=wa?n:Math.min(c,a),p=wa?r:Math.max(c,a),l.select(".dragarea").attr("x",k).attr("y",m).attr("width",h-k).attr("height",
p-m),l.selectAll(".-shapes").selectAll(".-shape").filter(function(a){return ma(a)}).each(function(a,b){var c=e.select(this),d=c.classed(na),g=c.classed(nc),f,l,rd,n;f=!1;"circle"===this.nodeName?(f=1*c.attr("cx"),l=1*c.attr("cy"),n=ld,f=k<f&&f<h&&m<l&&l<p):"rect"===this.nodeName&&(f=1*c.attr("x"),l=1*c.attr("y"),rd=1*c.attr("width"),n=nd,f=k<f+rd&&f<h&&l<p);f^g&&(c.classed(nc,!g),c.classed(na,!d),n(!d,c,a,b))}))}function yd(a){!B(f.data.targets)&&la&&(mc=a,l.select(".chart").append("rect").attr("class",
"dragarea").style("opacity",0.1),cb=!0)}function zd(){!B(f.data.targets)&&la&&(l.select(".dragarea").transition().duration(100).style("opacity",0).remove(),l.selectAll(".-shape").classed(nc,!1),cb=!1)}function V(a){var b,c,d,k;d=ve();var p,t,x,u,v,w=B(f.data.targets),A,s;a=y(a)?a:{};c=y(a.withY)?a.withY:!0;k=y(a.withSubchart)?a.withSubchart:!0;t=y(a.withTransition)?a.withTransition:!0;x=y(a.withTransform)?a.withTransform:!1;u=y(a.withUpdateXDomain)?a.withUpdateXDomain:!1;v=y(a.withUpdateOrgXDomain)?
...
{withTransition:t});Xa().forEach(function(a){Xb[a]=!0})}function Ze(){"mousemove"===e.event.sourceEvent.type&&E.altDomain?(m.domain(E.altDomain),E.scale(m).updateScaleExtent()):(ja&&m.orgDomain()[0]===$[0]&&m.domain([$[0]-1E-10,m.orgDomain()[1]]),V({withTransition:!1,withY:!1,withSubchart:!1}),"mousemove"===e.event.sourceEvent.type&&(lc=!0))}function Qe(){function a(){b.forEach(function(a){a()})}var b=[];a.add=function(a){b.push(a)};return a}function W(a){var b;a=y(a)?a:{};b=y(a.withTransition)?a.withTransition:
!1;a=y(a.withTransform)?a.withTransform:!1;qa();P();J.scale(T);ea&&E.scale(m);p.attr("width",Oa).attr("height",ra);p.select("#"+gc).select("rect").attr("width",q).attr("height",r);p.select("#xaxis-clip").select("rect").attr("width",ha);p.select(".zoom-rect").attr("width",q).attr("height",r);l.attr("transform",K.main);l.select(".x.axis").attr("transform",K.x);l.select(".y2.axis").attr("transform",K.y2);l.select(".chart-arcs").attr("transform",K.arc);Ha&&(G.attr("transform",K.context),G.select(".x.axis").attr("transform",
K.subx));var c=!1!==b?250:0;ia&&Ia.transition().duration(c).attr("transform",K.legend);V({withTransition:b,withUpdateXDomain:!0,withTransform:a,durationForExit:0})}function ud(a){var b;l.select(".chart-texts").selectAll(".chart-text").data(a).enter().append("g").attr("class",function(a){return"chart-text target target-"+a.id}).style("pointer-events","none").append("g").attr("class",me).style("fill",function(a){return I(a.id)});l.select(".chart-bars").selectAll(".chart-bar").data(a).enter().append("g").attr("class",
...
b.append("path").attr("class",pe).style("opacity",function(){Ad=+e.select(this).style("opacity");return 0}).style("fill",function(a){return I(a.id)});b.append("g").attr("class",function(a){return"selected-circles selected-circles-"+a.id});b.append("g").attr("class",ne).style("fill",function(a){return I(a.id)}).style("cursor",function(a){return ma(a)?"pointer":null});a.forEach(function(a){var b=yb(a.id);l.selectAll(".selected-circles"+b).selectAll(".selected-circle").each(function(b){b.value=a.values[b.x].value})});
b=l.select(".chart-arcs").selectAll(".chart-arc").data(Sb(a)).enter().append("g").attr("class",function(a){return"chart-arc target target-"+a.data.id});b.append("path").attr("class",oe).style("opacity",0).style("fill",function(a){return I(a.data.id)}).style("cursor",function(a){return ma(a)?"pointer":null}).each(function(a){this._current=a}).on("mouseover",function(a){wc(a.data.id);oc(a.data.id)}).on("mouseout",function(a){Ub(a.data.id);pc()});b.append("text").attr("dy",".35em").style("opacity",0).style("text-anchor",
...
r*b},m=function(){return 0.2*sa});h=function(a,b){return e(a,b)+14};n=function(a,b){return m(a,b)+9};l=function(a,b){return e(a,b)-4};p=function(a,b){return m(a,b)-7};u=function(b,c){var d=10*Math.ceil((b+30)/10),g=10*Math.ceil((c+4)/10);d>r&&(r=d,x||(s=(Qa-r*Object.keys(a).length)/2));g>q&&(q=g,x&&(s=(sa-q*Object.keys(a).length)/2))};d=Ia.selectAll(".legend-item").data(c).enter().append("g").attr("class",function(a){return"legend-item legend-item-"+a}).style("cursor","pointer").on("click",function(a){af(a)}).on("mouseover",
...
e+"</th></tr>"),l=a[f].name,h=c(a[f].value),e+="<tr class='-tooltip-name-"+a[f].id+"'><td class='name'><span style='background-color:"+d(a[f].id)+"'></span>"+l+"</td><td class='value'>"+h+"</td></tr>");return e+"</table>"}),Pe=d(["tooltip","init","show"],!1),Ja=d(["tooltip","init","x"],0),vd=d(["tooltip","init","position"],{top:"0px",left:"50px"}),gc=ya.replace("#","")+"-clip",bb="url("+document.URL+"#"+gc+")",C="timeseries"===sc,ja="categorized"===sc,Wb=!C&&(ua||ba),mc=null,cb=!1,lc=!1,I=function(a,
...
this.scaleExtent([b[0]*a,b[1]*a]);return this});var p,vb,l,G,Ia,U,ub,$,Ad,Xb={};f.focus=function(a){var b=p.selectAll(ka(a)),c=b.filter(qc),b=b.filter(rc);f.revert();f.defocus();c.classed("focused",!0).transition().duration(100).style("opacity",1);b.transition().duration(100).style("opacity",1);B(f.data.targets)&&wc(a,!0);oc(a)};f.defocus=function(a){var b=p.selectAll(ka(a)),c=b.filter(qc),b=b.filter(rc);f.revert();c.classed("focused",!1).transition().duration(100).style("opacity",0.3);b.transition().duration(100).style("opacity",
0.3);B(f.data.targets)&&Ub(a);$e(a)};f.revert=function(a){var b=p.selectAll(ka(a)),c=b.filter(qc),b=b.filter(rc);c.classed("focused",!1).transition().duration(100).style("opacity",1);b.transition().duration(100).style("opacity",1);B(f.data.targets)&&Ub(a);pc()};f.show=function(a){p.selectAll(ka(a)).transition().style("opacity",1)};f.hide=function(a){p.selectAll(ka(a)).transition().style("opacity",0)};f.unzoom=function(){J.clear().update();V({withUpdateXDomain:!0})};f.load=function(a){"function"!==
typeof a.done&&(a.done=function(){});a.xs&&ie(a.xs);"categories"in a&&ja&&(Sa=a.categories,S.categories(Sa));if("cacheIds"in a&&ee(a.cacheIds))db(fe(a.cacheIds),a.done);else if("data"in a)db(Wa(a.data),a.done);else if("url"in a)e.csv(a.url,function(b,c){db(Wa(c),a.done)});else if("rows"in a)db(Wa(Kc(a.rows)),a.done);else if("columns"in a)db(Wa(Lc(a.columns)),a.done);else throw Error("url or rows or columns is required.");};f.unload=function(a){f.data.targets=f.data.targets.filter(function(b){return b.id!==
a});p.selectAll(ka(a)).transition().style("opacity",0).remove();ia&&(p.selectAll(".legend-item"+yb(a)).remove(),xb(f.data.targets));0<f.data.targets.length&&V({withUpdateOrgXDomain:!0,withUpdateXDomain:!0})};f.selected=function(a){a=yb(a);return e.merge(l.selectAll(".-shapes"+a).selectAll(".-shape").filter(function(){return e.select(this).classed(na)}).map(function(a){return a.map(function(a){return a.__data__})}))};f.select=function(a,b,c){la&&l.selectAll(".-shapes").selectAll(".-shape").each(function(d,
f){var h="circle"===this.nodeName?kd:Ee,l="circle"===this.nodeName?ac:md;0<=b.indexOf(f)?ma(d)&&(wa||w(a)||0<=a.indexOf(d.id))&&h(e.select(this).classed(na,!0),d,f):y(c)&&c&&l(e.select(this).classed(na,!1),d,f)})};f.unselect=function(a,b){la&&l.selectAll(".-shapes").selectAll(".-shape").each(function(c,d){var f="circle"===this.nodeName?ac:md;(w(b)||0<=b.indexOf(d))&&ma(c)&&(wa||w(a)||0<=a.indexOf(c.id))&&f(e.select(this).classed(na,!1),c,d)})};f.toLine=function(a){ca(a,"line");W({withTransition:!0})};
