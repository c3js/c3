...
R=p.selectAll(".tick.major").data(ia,String),S=R.enter().insert("g","path").attr("class","tick major").style("opacity",1E-6),T=d.transition(R.exit()).style("opacity",1E-6).remove(),w=d.transition(R).style("opacity",1),F,N=u.rangeExtent?u.rangeExtent():P(u.range()),p=p.selectAll(".domain").data([0]);p.enter().append("path").attr("class","domain");var p=d.transition(p),y=u.copy(),da=this.__chart__||y;this.__chart__=y;S.append("line");S.append("text");var ja=S.select("line"),Va=w.select("line"),ka=R.select("text"),
...
g?Na:0).attr("x2",g?Na:q).attr("y1",g?0:Na).attr("y2",g?s:Na),c.selectAll("text").attr("x",g?0:q).attr("y",Na));c=function(a){var b=ea?n?1.2*J.tickOffset()/n:0:0.6*((g?s:q)*(a?1:ta(U)/ta(m.domain()))/(fb()-1)),c=Sd(b,n,f,!!a),e=Td(!!a),d=Ud(f,!!a),h=a?F:w;return function(f,k){var l=h(f.id)(0),m=d(f,k)||l,l=[[c(f),m],[c(f),e(f)-(l-m)],[c(f)+b,e(f)-(l-m)],[c(f)+b,m]],m=a||!g?0:1,n=a||!g?1:0;return"M "+l[0][m]+","+l[0][n]+" L"+l[1][m]+","+l[1][n]+" L"+l[2][m]+","+l[2][n]+" L"+l[3][m]+","+l[3][n]+" z"}};
b=l.selectAll(".-bars").selectAll(".-bar").data(Pc);b.enter().append("path").attr("d",c(!1)).style("stroke","none").style("opacity",0).style("fill",function(a){return K(a.id)}).attr("class",Bc);b.transition().duration(v).attr("d",c(!1)).style("opacity",1);b.exit().transition().duration(v).style("opacity",0).remove();l.selectAll(".chart-line").select(".-line").transition().duration(v).attr("d",re);l.selectAll(".-area").transition().duration(v).attr("d",se);b=l.selectAll(".-circles").selectAll(".-circle").data(Vd);
b.enter().append("circle").attr("class",Od).attr("r",Ub);b.transition().duration(v).style("opacity",Qd).attr("cx",g?xa:wa).attr("cy",g?wa:xa);b.exit().remove();l.selectAll(".chart-arc").select(".-arc").style("opacity",function(a){return a===this._current?0:1}).transition().duration(v).attrTween("d",function(a){a=ja(a);if(!a)return function(){return"M 0 0"};var b=d.interpolate(this._current,a);this._current=b(0);return function(a){a=b(a);return Oc(a.data)?kb(a,!0):"M 0 0"}}).style("opacity",1);l.selectAll(".chart-arc").select("text").attr("transform",
...
a).selectAll(".-shape").filter(function(){return d.select(this).classed(ha)}).map(function(a){return a.map(function(a){return a.__data__})}))};h.select=function(a,b,c){fa&&l.selectAll(".-shapes").selectAll(".-shape").each(function(e,f){var g="circle"===this.nodeName?Sc:Zd,h="circle"===this.nodeName?Qb:Uc;0<=b.indexOf(f)?ga(e)&&(pa||x(a)||0<=a.indexOf(e.id))&&g(d.select(this).classed(ha,!0),e,f):r(c)&&c&&h(d.select(this).classed(ha,!1),e,f)})};h.unselect=function(a,b){fa&&l.selectAll(".-shapes").selectAll(".-shape").each(function(c,
