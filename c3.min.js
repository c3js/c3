(function(K){function xd(){function R(e,g){e.attr("transform",function(e){return"translate("+(g(e)+oa)+", 0)"})}function x(e,g){e.attr("transform",function(e){return"translate(0,"+g(e)+")"})}function y(e){var g=e[0];e=e[e.length-1];return g<e?[g,e]:[e,g]}function z(e){for(var g=[],q=Math.ceil(e[0]);q<e[1];q++)g.push(q);0<g.length&&0<g[0]&&g.unshift(g[0]-(g[1]-g[0]));return g}function K(e){e=e<L.length?L[e]:e;return S?S(e):e}function p(p){p.each(function(){var p=e.select(this),ea=z(q.domain()),Y=p.selectAll(".tick.major").data(ea,
String),fa=Y.enter().insert("g","path").attr("class","tick major").style("opacity",1E-6),L=e.transition(Y.exit()).style("opacity",1E-6).remove(),ga=e.transition(Y).style("opacity",1),P,T=q.rangeExtent?q.rangeExtent():y(q.range()),p=p.selectAll(".domain").data([0]);p.enter().append("path").attr("class","domain");var p=e.transition(p),s=q.copy(),ha=this.__chart__||s;this.__chart__=s;fa.append("line");fa.append("text");var S=fa.select("line"),U=ga.select("line"),pa=Y.select("text"),ab=fa.select("text"),
qa=ga.select("text");oa=(s(1)-s(0))/2;Y=xa?0:oa;switch(C){case "bottom":P=R;S.attr("y2",M);ab.attr("y",Math.max(M,0)+g);U.attr("x1",Y).attr("x2",Y).attr("y2",M);qa.attr("x",0).attr("y",Math.max(M,0)+g);pa.attr("dy",".71em").style("text-anchor","middle");pa.text(function(e){return ea.length<V||0===e%Math.ceil(ea.length/V)?K(e):""});p.attr("d","M"+T[0]+","+Z+"V0H"+T[1]+"V"+Z);break;case "left":P=x,S.attr("x2",-M),ab.attr("x",-(Math.max(M,0)+g)),U.attr("x2",-M).attr("y2",0),qa.attr("x",-(Math.max(M,
0)+g)).attr("y",oa),pa.attr("dy",".32em").style("text-anchor","end"),pa.text(function(e){return ea.length<V||0===e%Math.ceil(ea.length/V)?K(e):""}),p.attr("d","M"+-Z+","+T[0]+"H0V"+T[1]+"H"+-Z)}if(q.ticks)fa.call(P,ha),ga.call(P,s),L.call(P,s);else{var yb=s.rangeBand()/2,L=function(e){return s(e)+yb};fa.call(P,L);ga.call(P,L)}})}var q=e.scale.linear(),C="bottom",M=6,Z=6,g=3,xa=!1,V=10,oa=0,S=null,L=[];p.scale=function(e){if(!arguments.length)return q;q=e;return p};p.orient=function(e){if(!arguments.length)return C;
...
a.tickOffset=function(){return 0};return a}function U(a){return a in zc?zc[a]:"y"}function pa(){var a=D?Hd:ja?Id:null;kb&&("function"===typeof kb?a=kb:D&&(a=function(a){return e.time.format(kb)(a)}));return a}function ab(){var a=Ka();return 10>a?a:10}function qa(a){var b=!1;Pb(k.data.targets).forEach(function(c){b||c.data.id!==a.data.id||(b=!0,a=c)});return b?a:null}function yb(){var a=e.svg.arc().outerRadius(Ja).innerRadius(Fb),b=function(b,d){var f;return d?a(b):(f=qa(b))?a(f):"M 0 0"};b.centroid=
...
Ub(a,b);if(e<c||!c)c=e,d=a});return d}function Wc(a,b){var c,d,f,e,g;e=a.filter(function(a){return a&&z(a.value)});Xc&&0!==e.length&&(X.html(Yc(a)).style("visibility","hidden").style("display","block"),c=X.property("offsetWidth"),d=X.property("offsetHeight"),h?e=b[0]:(f=oa(),e=f+(bb?bb:cb?1:40)+m(e[0].x)+20,g=e+c,f=f+(Ia?Ia:L())-(db?db:Cb?eb||h?1:40:1),g>f&&(e-=c+30)),c=b[1]+15+d<S()?b[1]+15:b[1]-d,X.style("top",c+"px").style("left",e+"px").style("visibility","visible"))}function Zc(a){a=a.filter(function(a){return a&&
...
" "+c(l(f))+" "+b(k(f+e))+" "+c(l(f+e))};for(f=0;f<a.length;f++)if(x(ua)||!oe(a[f].x,ua))g+=" "+jd(a[f])+" "+n(a[f]);else{k=P(a[f-1].x,a[f].x);l=T(a[f-1].value,a[f].value);e=b(a[f].x)-b(a[f-1].x);m=c(a[f].value)-c(a[f-1].value);e=Math.sqrt(Math.pow(e,2)+Math.pow(m,2));m=2/e;var p=2*m;for(e=m;1>=e;e+=p)g+=d(a[f-1],a[f],e,m)}return g}function cc(a){var b;rb=e.select(za);if(rb.empty())K.alert('No bind element found. Check the selector specified by "bindto" and existance of that element. Default "bindto" is "#chart".');
else{rb.html("");k.data.x={};k.data.targets=Qa(a);xa();sc();m.domain(e.extent(nb(k.data.targets)));A.domain(lb("y"));aa.domain(lb("y2"));W.domain(m.domain());hb.domain(A.domain());ib.domain(aa.domain());$=m.domain();I.scale(W);va&&F.scale(m);n=e.select(za).append("svg").attr("width",r+C+Z).attr("height",t+q+M).on("mouseenter",te).on("mouseleave",ue);sb=n.append("defs");sb.append("clipPath").attr("id",dc).append("rect").attr("y",q).attr("width",r).attr("height",t-q);sb.append("clipPath").attr("id",
"xaxis-clip").append("rect").attr("x",-1-C).attr("y",-20).attr("width",ea).attr("height",zb);sb.append("clipPath").attr("id","yaxis-clip").append("rect").attr("x",-C+1).attr("y",q-1).attr("width",yd).attr("height",Y);l=n.append("g").attr("transform",J.main);G=Ea?n.append("g").attr("transform",J.context):null;Fa=Ga?n.append("g").attr("transform",J.legend):null;X=e.select(za).style("position","relative").append("div").style("position","absolute").style("z-index","10").style("display","none");l.append("g").attr("class",
"x axis").attr("clip-path",h?"":"url("+document.URL+"#xaxis-clip)").attr("transform",J.x).call(h?La:N).append("text").attr("class","-axis-x-label").attr("x",r).attr("dy","-.5em").style("text-anchor","end").text(ve);l.append("g").attr("class","y axis").attr("clip-path",h?"url("+document.URL+"#yaxis-clip)":"").call(h?N:La).append("text").attr("transform","rotate(-90)").attr("dy","1.2em").attr("dx","-.5em").style("text-anchor","end").text(we);Cb&&l.append("g").attr("class","y2 axis").attr("transform",
...
"chart-lines"),G.append("g").attr("clip-path",Xa).attr("class","x brush").call(I).selectAll("rect").attr(h?"width":"height",h?Db:fb),G.append("g").attr("class","x axis").attr("transform",J.subx).attr("clip-path",h?"url("+document.URL+"#yaxis-clip)":"").call(gb));Ga&&ub(k.data.targets);md(k.data.targets);w({withTransform:!0,withUpdateXDomain:!0});if(Ae){if(D&&"string"===typeof Ha){Ha=sa(Ha);for(a=0;a<k.data.targets[0].values.length&&0!==k.data.targets[0].values[a].x-Ha;a++);Ha=a}X.html(Yc(k.data.targets.map(function(a){return pb(a.values[Ha])})));
X.style("top",nd.top).style("left",nd.left).style("display","block")}null==K.onresize&&(K.onresize=Be());K.onresize.add&&K.onresize.add(od)}}function Ce(a){a.append("rect").attr("class",Pc).style("cursor",la&&wa?"pointer":null).on("mouseover",function(a,c){if(!Ya&&!E(k.data.targets)){var d=k.data.targets.map(function(a){return pb(a.values[c])}),f,e;if(0<Object.keys(Pa).length){e=[];for(var g in Pa)for(f=0;f<d.length;f++)if(d[f].id===g){e.push(d[f]);d.shift(f);break}d=e.concat(d)}gc&&$b(c,void 0).classed(da,
...
"hidden").attr("y2",t);if(kd){if("year"===Ge){b=[];var s=nb();a=s[0].getFullYear();for(s=s[1].getFullYear();a<=s;a++)b.push(new Date(a+"-01-01 00:00:00"))}else b=m.ticks(10);b=l.select(".xgrids").selectAll(".xgrid").data(b);b.enter().append("line").attr("class","xgrid");b.attr("x1",h?0:function(a){return m(a)-N.tickOffset()}).attr("x2",h?r:function(a){return m(a)-N.tickOffset()}).attr("y1",h?function(a){return m(a)-N.tickOffset()}:q).attr("y2",h?function(a){return m(a)-N.tickOffset()}:t).style("opacity",
...
1E-10,m.orgDomain()[1]]),w({withTransition:!1,withY:!1,withSubchart:!1}),"mousemove"===e.event.sourceEvent.type&&(ic=!0))}function Be(){function a(){b.forEach(function(a){a()})}var b=[];a.add=function(a){b.push(a)};return a}function od(){xa();sc();I.scale(W);va&&F.scale(m);n.attr("width",Ab).attr("height",ya);n.select("#"+dc).select("rect").attr("width",r).attr("height",t);n.select("#xaxis-clip").select("rect").attr("width",ea);n.select(".zoom-rect").attr("width",r).attr("height",t);l.select(".x.axis").attr("transform",
...
function(a){return O(a.id)});b.append("g").attr("class",function(a){return"selected-circles selected-circles-"+a.id});b.append("g").attr("class",de).style("fill",function(a){return O(a.id)}).style("cursor",function(a){return ma(a)?"pointer":null});a.forEach(function(a){var b=wb(a.id);l.selectAll(".selected-circles"+b).selectAll(".selected-circle").each(function(b){b.value=a.values[b.x].value})});b=l.select(".chart-arcs").selectAll(".chart-arc").data(Pb(a)).enter().append("g").attr("class",function(a){return"chart-arc target target-"+
...
1);E(k.data.targets)&&Ac(a,!0);lc(a)};k.defocus=function(a){var b=n.selectAll(ka(a)),c=b.filter(nc),b=b.filter(oc);k.revert();c.classed("focused",!1).transition().duration(100).style("opacity",0.3);b.transition().duration(100).style("opacity",0.3);E(k.data.targets)&&Rb(a);Le(a)};k.revert=function(a){var b=n.selectAll(ka(a)),c=b.filter(nc),b=b.filter(oc);c.classed("focused",!1).transition().duration(100).style("opacity",1);b.transition().duration(100).style("opacity",1);E(k.data.targets)&&Rb(a);mc()};
k.show=function(a){n.selectAll(ka(a)).transition().style("opacity",1)};k.hide=function(a){n.selectAll(ka(a)).transition().style("opacity",0)};k.unzoom=function(){I.clear().update();w({withUpdateXDomain:!0})};k.load=function(a){"function"!==typeof a.done&&(a.done=function(){});a.xs&&ae(a.xs);"categories"in a&&ja&&(Ma=a.categories,N.categories(Ma));if("cacheIds"in a&&Xd(a.cacheIds))Za(Yd(a.cacheIds),a.done);else if("data"in a)Za(Qa(a.data),a.done);else if("url"in a)e.csv(a.url,function(b,c){Za(Qa(c),
...
wb(a);return e.merge(l.selectAll(".-shapes"+a).selectAll(".-shape").filter(function(){return e.select(this).classed(na)}).map(function(a){return a.map(function(a){return a.__data__})}))};k.select=function(a,b,c){la&&l.selectAll(".-shapes").selectAll(".-shape").each(function(d,f){var g="circle"===this.nodeName?fd:re,h="circle"===this.nodeName?Yb:hd;0<=b.indexOf(f)?ma(d)&&(wa||x(a)||0<=a.indexOf(d.id))&&g(e.select(this).classed(na,!0),d,f):y(c)&&c&&h(e.select(this).classed(na,!1),d,f)})};k.unselect=
function(a,b){la&&l.selectAll(".-shapes").selectAll(".-shape").each(function(c,d){var f="circle"===this.nodeName?Yb:hd;(x(b)||0<=b.indexOf(d))&&ma(c)&&(wa||x(a)||0<=a.indexOf(c.id))&&f(e.select(this).classed(na,!1),c,d)})};k.toLine=function(a){ca(a,"line");w()};k.toSpline=function(a){ca(a,"spline");w()};k.toBar=function(a){ca(a,"bar");w()};k.toScatter=function(a){ca(a,"scatter");w()};k.toArea=function(a){ca(a,"area");w()};k.toAreaSpline=function(a){ca(a,"area-spline");w()};k.toPie=function(a){ca(a,
...
b.classes.indexOf(a)})});return Q};k.data.get=function(a){a=k.data.getAsTarget(a);return y(a)?a.values.map(function(a){return a.value}):void 0};k.data.getAsTarget=function(a){var b=mb(function(b){return b.id===a});return 0<b.length?b[0]:void 0};k.resize=function(a){Ia=a?a.width:null;Gb=a?a.height:null;od()};k.destroy=function(){k.data.targets=void 0;k.data.x={};rb.html("");K.onresize=null};if("url"in R.data)e.csv(R.data.url,function(a,b){cc(b)});else if("rows"in R.data)cc(Gc(R.data.rows));else if("columns"in
R.data)cc(Hc(R.data.columns));else throw Error("url or rows or columns is required.");return k}})(window);
