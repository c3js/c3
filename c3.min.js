...
va&&(q=k(va.top)?va.top:c,e=k(va.bottom)?va.bottom:c);"y2"===b&&null!==wa&&(q=k(wa.top)?wa.top:c,e=k(wa.bottom)?wa.bottom:c);return[lc(d,"bar")?0:g-e,f+q]}function L(){var a,b;if(la){if(ma.empty())return 1;a=y.domain();b=ma.extent()}else a=Cb,b=m.domain();return(a[1]-a[0])/(b[1]-b[0])}function ga(a){for(var b=0;b<a.length;b++)if(!(a[b]in La))return!1;return!0}function ha(a){for(var b=[],d=0;d<a.length;d++)a[d]in La&&b.push(sa(La[a[d]]));return b}function A(a){return a in Db?Db[a]:"y"}function F(a){return"y2"===
...
function(a){return a.values.length})}function ta(a){a=u(a)?l.data.targets:a;return a.map(function(a){return a.id})}function Va(a){var b=ta(),d;for(d=0;d<b.length;d++)if(b[d]===a)return!0;return!1}function Ka(a){return k(a)?l.data.targets.filter(a):l.data.targets}function ab(a){return"-shapes -shapes-"+a.id}function Eb(a){return ab(a)+" -line -line-"+a.id}function nc(a){return ab(a)+" -circles -circles-"+a.id}function Fb(a){return ab(a)+" -bars -bars-"+a.id}function oc(a,b){return"-shape -shape-"+
b+" -circle -circle-"+b}function Gb(a,b){return"-shape -shape-"+b+" -bar -bar-"+b}function pc(a,b){return"region region-"+b+" "+("classes"in a?[].concat(a.classes).join(" "):"")}function Oa(a){return m(a.x)}function ya(a){return m(H?ba(a.value):a.value)}function Hb(a){return x(a.value)}function U(a){return m(a.x)}function V(a){return F(a.id)(a.value)}function qc(){var a={},b=0,d,g;Ka(ua).forEach(function(c){for(d=0;d<r.length;d++)if(!(0>r[d].indexOf(c.id)))for(g=0;g<r[d].length;g++)if(r[d][g]in a){a[c.id]=
a[r[d][g]];break}u(a[c.id])&&(a[c.id]=b++)});a.__max__=b-1;return a}function Ib(a,b,d,g){var c=g?y:m;return function(g){var q=g.id in d?d[g.id]:0;return c(g.x)-a*(b/2-q)}}function Jb(a,b,d,g){var c=Object.keys(b);return function(e,q){var k=0,h=g?B(e.id):F(e.id);Ka(ua).forEach(function(d){d.id!==e.id&&b[d.id]===b[e.id]&&c.indexOf(d.id)<c.indexOf(e.id)&&(k+=a(d.values[q]))});return d?k:h(e.value)-k}}function Kb(a,b){var d=null===a?function(a){return a}:function(b){return a-b};return function(a){var c=
b?B(a.id):F(a.id);return d(c(a.value))}}function bb(a,b){var d=u(a)?ta():a;"string"===typeof d&&(d=[d]);for(var c=0;c<d.length;c++)W[d[c]]=b}function lc(a,b){var d=!1;a.forEach(function(a){W[a.id]===b&&(d=!0);a.id in W||"line"!==b||(d=!0)});return d}function cb(a){a="string"===typeof a?a:a.id;return!(a in W)||"line"===W[a]||"spline"===W[a]}function ua(a){return"bar"===W["string"===typeof a?a:a.id]}function rc(a){return cb(a)?a.values:[]}function Lb(a){return ua(a)?a.values:[]}function Mb(a,b){var d=
...
a.length;f++)if(u(p)||!sc(a[f].x,p))q+=" "+Sb(a[f])+" "+n(a[f]);else for(h=N(a[f-1].x,a[f].x),l=w(a[f-1].value,a[f].value),s=b(a[f].x)-b(a[f-1].x),m=d(a[f].value)-d(a[f-1].value),s=Math.sqrt(Math.pow(s,2)+Math.pow(m,2)),m=2/s,diffx2=2*m,s=m;1>=s;s+=diffx2)q+=c(a[f-1],a[f],s,m);return q}function Tb(){return"translate("+(e?0:t)+","+(e?10:0)+")"}function eb(a){var b=l.data.targets=M(a),d,c;m.domain(d3.extent(a.map(function(a){return a.x})));x.domain(E(b,"y"));Q.domain(E(b,"y2"));y.domain(m.domain());
Ha.domain(x.domain());Ia.domain(Q.domain());D.ticks(10>a.length?a.length:10);ja.ticks(xc).outerTickSize(0).tickFormat(yc);Ja.ticks(zc).outerTickSize(0).tickFormat(Ac);Cb=m.domain();fb.x(m);h.append("g").attr("class","x axis").attr("clip-path",e?"":"url(#xaxis-clip)").attr("transform","translate(0,"+v+")").call(e?ja:D);h.append("g").attr("class","y axis").attr("clip-path",e?"url(#yaxis-clip)":"").call(e?D:ja).append("text").attr("transform","rotate(-90)").attr("dy","1.4em").attr("dx","-.8em").style("text-anchor",
"end").text(Bc);vb&&h.append("g").attr("class","y2 axis").attr("transform",Tb).call(Ja);d=h.append("g").attr("clip-path",Pa).attr("class","grid");Ub&&d.append("g").attr("class","xgrids");gb&&(c=d.append("g").attr("class","xgrid-lines").selectAll(".xgrid-line").data(gb).enter().append("g").attr("class","xgrid-line"),c.append("line").attr("class",function(a){return""+a["class"]}),c.append("text").attr("class",function(a){return""+a["class"]}).attr("text-anchor","end").attr("transform",e?"":"rotate(-90)").attr("dx",
e?0:-p).attr("dy",-6).text(function(a){return a.text}));Cc&&d.append("g").attr("class","xgrid-focus").append("line").attr("class","xgrid-focus").attr("x1",e?0:-10).attr("x2",e?t:-10).attr("y1",e?-10:p).attr("y2",e?-10:v);Vb&&d.append("g").attr("class","ygrids");hb&&d.append("g").attr("class","ygrid-lines").selectAll("ygrid-line").data(hb).enter().append("line").attr("class",function(a){return"ygrid-line "+a["class"]});h.append("g").attr("clip-path",Pa).attr("class","regions");h.append("g").attr("clip-path",
...
jb[0];var b=jb[1],d=d3.mouse(this),c=d[0],d=d[1],g=Math.min(a,c),e=Math.max(a,c),k=ca?p:Math.min(b,d),l=ca?v:Math.max(b,d);h.select(".dragarea").attr("x",g).attr("y",k).attr("width",e-g).attr("height",l-k);h.selectAll(".-shapes").selectAll(".-shape").filter(function(a){return ea(a)}).each(function(a,b){var d=d3.select(this),c=d.classed(Z),f=d.classed(kb),h,q,s,m;h=!1;"circle"===this.nodeName?(h=1*d.attr("cx"),q=1*d.attr("cy"),m=Pb,h=g<h&&h<e&&k<q&&q<l):"rect"===this.nodeName&&(h=1*d.attr("x"),q=1*
d.attr("y"),s=1*d.attr("width"),m=Rb,h=g<h+s&&h<e&&q<l);h^f&&(d.classed(kb,!f),d.classed(Z,!c),m(!c,d,a,b))})}}).on("dragstart",function(){X&&(jb=d3.mouse(this),h.select(".chart").append("rect").attr("class","dragarea").style("opacity",0.1),Qa=!0)}).on("dragend",function(){X&&(h.select(".dragarea").transition().duration(100).style("opacity",0).remove(),h.selectAll(".-shape").classed(kb,!1),Qa=!1)})).call(fb);h.select(".chart").append("g").attr("class","chart-bars");h.select(".chart").append("g").attr("class",
"chart-lines");la&&(O.append("g").attr("clip-path",Pa).attr("class","chart"),O.select(".chart").append("g").attr("class","chart-bars"),O.select(".chart").append("g").attr("class","chart-lines"),null!==lb&&ma.extent(H?lb(Na,$a):lb(0,$()-1)),O.append("g").attr("class","x brush").call(ma).selectAll("rect").attr("height",ia),O.append("g").attr("class","x axis").attr("transform","translate(0,"+ia+")").call(ka));na&&Ra(b);Zb(b);I({withTransition:!1});if(Ec){if(H&&"string"==typeof oa){oa=ba(oa);for(a=0;a<
...
a.enter().append("line").attr("class","xgrid");a.exit().remove();h.selectAll(".xgrid").attr("x1",function(a){return m(a)-D.tickOffset()}).attr("x2",function(a){return m(a)-D.tickOffset()}).attr("y1",p).attr("y2",v)}gb&&(a=h.selectAll(".xgrid-lines"),a.selectAll("line").attr("x1",e?0:ya).attr("x2",e?t:ya).attr("y1",e?ya:p).attr("y2",e?ya:v),a.selectAll("text").attr("x",e?t:0).attr("y",ya));b&&Vb&&(ygrid=h.select(".ygrids").selectAll(".ygrid").data(x.ticks(10)),ygrid.enter().append("line").attr("class",
"ygrid"),ygrid.attr("x1",e?x:0).attr("x2",e?x:t).attr("y1",e?0:x).attr("y2",e?v:x).attr("opacity",0).transition().attr("opacity",1),ygrid.exit().remove());b&&hb&&h.select(".ygrid-lines").selectAll(".ygrid-line").attr("y1",Hb).attr("y2",Hb);s=fa?1.2*D.tickOffset()/c:0.6*((e?v:t)*L()/($()-1));q=Kb(e?null:v);a=Ib(s,c,d);f=Jb(q,d,e);b=h.selectAll(".-bars").selectAll(".-bar").data(Lb);b.transition().duration(r?250:0).attr("x",e?f:a).attr("y",e?a:f).attr("width",e?q:s).attr("height",e?s:q);b.enter().append("rect").attr("class",
Gb).attr("x",e?f:a).attr("y",e?a:f).attr("width",e?q:s).attr("height",e?s:q).style("opacity",0).transition().duration(r?250:0).style("opacity",1);b.exit().transition().duration(r?250:0).style("opacity",0).remove();h.selectAll(".-line").transition().duration(r?250:0).attr("d",Gc);b=h.selectAll(".-circles").selectAll(".-circle").data(rc);b.transition().duration(r?250:0).attr("cx",e?V:U).attr("cy",e?U:V);b.enter().append("circle").attr("class",oc).attr("cx",e?V:U).attr("cy",e?U:V).attr("r",ib);b.exit().remove();
u&&la&&(s=fa?1.2*ka.tickOffset()/c:0.6*((e?v:t)*L()/($()-1)),q=Kb(ia,!0),a=Ib(s,c,d,!0),f=Jb(q,d,!1,!0),contextBar=O.selectAll(".-bars").selectAll(".-bar").data(Lb),contextBar.transition().duration(r?250:0).attr("x",a).attr("y",f).attr("width",s).attr("height",q),contextBar.enter().append("rect").attr("class",Gb).attr("x",a).attr("y",f).attr("width",s).attr("height",q).style("opacity",0).transition().style("opacity",1),contextBar.exit().transition().style("opacity",0).remove(),O.selectAll(".-line").transition().duration(r?
...
b=b.enter().append("g").attr("class",function(a){return"chart-line target target-"+a.id}).style("opacity",0),b.append("path").attr("class",Eb).style("stroke",function(a){return R(a.id)}));na&&Ra(a);d3.selectAll(".target").transition().style("opacity",1)}function Aa(a,b){l.data.targets.forEach(function(b){for(var c=0;c<a.length;c++)if(b.id===a[c].id){b.values=a[c].values;a.splice(c,1);break}});l.data.targets=l.data.targets.concat(a);Zb(l.data.targets);I();b()}function Ra(a,b){var d=ta(a),c,f=t/2-Ba*
...
250:0).attr("x",function(a,b){return f+Ba*b});Ca.selectAll("rect.legend-item-tile").data(d).transition().duration(e?250:0).attr("x",function(a,b){return f+Ba*b});Ca.selectAll("text").data(d).transition().duration(e?250:0).attr("x",function(a,b){return f+Ba*b+14})}function Da(a){return k(a)?".target-"+a:".target"}var l={data:{}},La={},Y="_expanded_",Z="_selected_",kb="_included_";if(!("bindto"in z))throw Error("bindto is required in config");var zb=c(["size","width"],null),Ga=c(["size","height"],280);
if(!("data"in z))throw Error("data is required in config");var Ma=c(["data","x"],"x"),Jc=c(["data","x_format"],"%Y-%m-%d"),mc=c(["data","id_converter"],function(a){return a}),xa=c(["data","names"],{}),r=c(["data","groups"],[]),Db=c(["data","axes"],{}),W=c(["data","types"],{}),bc=c(["data","regions"],{}),Kc=c(["data","colors"],{}),X=c(["data","selection","enabled"],!1),ca=c(["data","selection","grouped"],!1),ea=c(["data","selection","isselectable"],function(a){return!0}),la=c(["subchart","show"],!1),
tb=la?c(["subchart","size","height"],60):0,lb=c(["subchart","default"],null),Lc=c(["color","pattern"],null),na=c(["legend","show"],!0),Ba=c(["legend","item","width"],80),Ic=c(["legend","item","onclick"],function(){}),cc=c(["axis","x","type"],"indexed"),Sa=c(["axis","x","categories"],[]),dc=c(["axis","x","tick","centered"],!1),ic=c(["axis","y","max"],null),gc=c(["axis","y","min"],null),kc=c(["axis","y","center"],null),Bc=c(["axis","y","text"],null);c(["axis","y","rescale"],!0);var Xa=c(["axis","y",
"inner"],!1),yc=c(["axis","y","format"],function(a){return a}),va=c(["axis","y","padding"],null),xc=c(["axis","y","ticks"],10),vb=c(["axis","y2","show"],!1),hc=c(["axis","y2","max"],null),fc=c(["axis","y2","min"],null),jc=c(["axis","y2","center"],null);c(["axis","y2","text"],null);c(["axis","y2","rescale"],!0);var Wa=c(["axis","y2","inner"],!1),Ac=c(["axis","y2","format"],function(a){return a}),wa=c(["axis","y2","padding"],null),zc=c(["axis","y2","ticks"],10),e=c(["axis","rotated"],!1),Ub=c(["grid",
"x","show"],!1),Fc=c(["grid","x","type"],"tick"),gb=c(["grid","x","lines"],null),Vb=c(["grid","y","show"],!1);c(["grid","y","type"],"tick");var hb=c(["grid","y","lines"],null),ib=c(["point","show"],!0)?c(["point","r"],2.5):0,Cc=c(["point","focus","line","enabled"],!0),Wb=c(["point","focus","expand","enabled"],!0),Xb=c(["point","focus","expand","r"],Wb?4:ib),za=c(["point","focus","select","r"],8),Dc=c(["point","onclick"],function(){}),tc=c(["point","onselected"],function(){}),uc=c(["point","onunselected"],
...
Ec=c(["tooltip","init","show"],!1),oa=c(["tooltip","init","x"],0),$b=c(["tooltip","init","position"],{top:"0px",left:"50px"}),nb=z.bindto.replace("#","")+"-clip",Pa="url(#"+nb+")",H="timeseries"===cc,fa="categorized"===cc,jb=null,Qa=!1,aa=na?40:0,Mc=function(a){return function(b){for(var d=a.length-1,c=a[d];!c[1](b);)c=a[--d];return c[0](b)}}([[d3.time.format("%Y/%-m/%-d"),function(){return!0}],[d3.time.format("%-m/%-d"),function(a){return a.getMonth()}],[d3.time.format("%-m/%-d"),function(a){return 1!=
a.getDate()}],[d3.time.format("%-m/%-d"),function(a){return a.getDay()&&1!=a.getDate()}],[d3.time.format("%I %p"),function(a){return a.getHours()}],[d3.time.format("%I:%M"),function(a){return a.getMinutes()}],[d3.time.format(":%S"),function(a){return a.getSeconds()}],[d3.time.format(".%L"),function(a){return a.getMilliseconds()}]]),ba=d3.time.format(Jc).parse,sb,xb,ub,wb,yb,t,v,ia,Ab,Bb,Ya,Za,m,x,Q,y,Ha,Ia,D=fa?qb():d3.svg.axis(),ja=d3.svg.axis(),Ja=d3.svg.axis(),ka=fa?qb():d3.svg.axis();P();S();
...
" "+B(b.id)(b.values[0].value)}}(),mb=function(a){return"start"in a?m(H?ba(a.start):a.start):0},ac=function(a){var b=mb(a);a=("end"in a?m(H?ba(a.end):a.end):t)-b;return 0>a?0:a},R=function(a,b){var c=[],g=null!==b?b:"#1f77b4 #ff7f0e #2ca02c #d62728 #9467bd #8c564b #e377c2 #7f7f7f #bcbd22 #17becf".split(" ");return function(b){if(b in a)return a[b];0<=c.indexOf(b)||c.push(b);return g[c.indexOf(b)%g.length]}}(Kc,Lc),Ta=d3.select(z.bindto).append("svg").attr("width",t+G+C).attr("height",v+p+T),ob=Ta.append("defs");
ob.append("clipPath").attr("id",nb).append("rect").attr("y",p).attr("width",t).attr("height",v-p);ob.append("clipPath").attr("id","xaxis-clip").append("rect").attr("x",-1).attr("y",-1).attr("width",t+2).attr("height",40);ob.append("clipPath").attr("id","yaxis-clip").append("rect").attr("x",-G+1).attr("y",p-1).attr("width",G).attr("height",v-p+2);var h=Ta.append("g").attr("transform","translate("+G+","+p+")"),O=null;la&&(O=Ta.append("g").attr("transform","translate("+J+","+pa+")"));var Ca=null;na&&
(Ca=Ta.append("g").attr("transform","translate("+Ea+","+rb+")"));var da=d3.select(z.bindto).style("position","relative").append("div").style("position","absolute").style("width","30%").style("z-index","10").style("visibility","hidden"),ma=d3.svg.brush().x(y).on("brush",function(){I({withTransition:!1,withY:!1,withSubchart:!1})}),fb=d3.behavior.zoom().on("zoom",function(){I({withTransition:!1,withY:!1,withSubchart:!1,withUpdateXDomain:!1})}),Na=null,$a=null,Cb;l.focus=function(a){l.defocus();d3.selectAll(Da(a)).filter(function(a){return Va(a.id)}).classed("focused",
...
Ka(function(b){return b.id==a});return 0<b.length?b[0]:void 0};if("url"in z.data)d3.csv(z.data.url,function(a,b){eb(b)});else if("rows"in z.data)eb(qa(z.data.rows));else if("columns"in z.data)eb(ra(z.data.columns));else throw Error("url or rows or columns is required.");pb.onresize=function(){P();S();fb.x(m);d3.select("svg").attr("width",t+G+C);d3.select("#"+nb).select("rect").attr("width",t);d3.select("#xaxis-clip").select("rect").attr("width",t+2);d3.select("g.y2.axis").attr("transform",Tb);na&&
Ra(l.data.targets,{withTransition:!1});I({withTransition:!1})};return l}})(window);
