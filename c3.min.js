...
function(a){return a.value})})}function na(a){var c={},d,p;a.forEach(function(a){c[a.id]=[];a.values.forEach(function(d){c[a.id].push(d.value)})});for(d=0;d<q.length;d++)for(p=1;p<q[d].length;p++)Aa(q[d][p])&&(u(c[q[d][p]])||c[q[d][p]].forEach(function(a,b){A(q[d][p])===A(q[d][0])&&(c[q[d][0]][b]+=1*a)}));return d3.max(Object.keys(c).map(function(a){return d3.max(c[a])}))}function B(a,c){var d=Ta(function(a){return A(a.id)===c}),p="y2"===c?qc:rc,f="y2"===c?sc:tc,p=null!==p?p:ma(d),f=null!==f?f:na(d),
b=0.1*Math.abs(f-p),n=b,e=b,g="y2"===c?uc:vc;null!==g&&(yDomainAbs=Math.max(Math.abs(p),Math.abs(f)),f=yDomainAbs-g,p=g-yDomainAbs);"y"===c&&null!==Ba&&(n=l(Ba.top)?Ba.top:b,e=l(Ba.bottom)?Ba.bottom:b);"y2"===c&&null!==Ca&&(n=l(Ca.top)?Ca.top:b,e=l(Ca.bottom)?Ca.bottom:b);return[wc(d,"bar")?0:p-e,f+n]}function T(a){var c;a?(a=m.domain(),c=E.extent()):(a=P,c=m.domain());return(a[1]-a[0])/(c[1]-c[0])}function K(a){for(var c=0;c<a.length;c++)if(!(a[c]in Ua))return!1;return!0}function ab(a){for(var c=
[],d=0;d<a.length;d++)a[d]in Ua&&c.push(zb(Ua[a[d]]));return c}function oa(a){return"start"in a?m(D?ia(a.start):a.start):0}function va(a){var c=oa(a);a=("end"in a?m(D?ia(a.end):a.end):t)-c;return 0>a?0:a}function ca(a){var c=Da[a.id];a.name=l(c)?c:a.id;return a}function wa(a){var c=a[0],d={},b=[],f,e;for(f=1;f<a.length;f++){d={};for(e=0;e<a[f].length;e++)d[c[e]]=a[f][e];b.push(d)}return b}function xa(a){var c=[],d,b,f;for(d=0;d<a.length;d++)for(f=a[d][0],b=1;b<a[d].length;b++)u(c[b-1])&&(c[b-1]={}),
...
function Bc(){var a={},c=0,d,b;Ta(Aa).forEach(function(f){for(d=0;d<q.length;d++)if(!(0>q[d].indexOf(f.id)))for(b=0;b<q[d].length;b++)if(q[d][b]in a){a[f.id]=a[q[d][b]];break}u(a[f.id])&&(a[f.id]=c++)});a.__max__=c-1;return a}function Qb(a,c,d,b){var f=b?V:m;return function(b){var p=b.id in d?d[b.id]:0;return f(b.x)-a*(c/2-p)}}function Rb(a,c,d,b){var f=Object.keys(c);return function(e,n){var g=0,k=b?Na(e.id):h(e.id);Ta(Aa).forEach(function(d){d.id!==e.id&&c[d.id]===c[e.id]&&f.indexOf(d.id)<f.indexOf(e.id)&&
(g+=a(d.values[n]))});return d?g:k(e.value)-g}}function Sb(a,c){var d=null===a?function(a){return a}:function(c){return a-c};return function(a){var b=c?Na(a.id):h(a.id);return d(b(a.value))}}function lb(a,c){var d=u(a)?ib():a;"string"===typeof d&&(d=[d]);for(var b=0;b<d.length;b++)Y[d[b]]=c}function wc(a,c){var d=!1;a.forEach(function(a){Y[a.id]===c&&(d=!0);a.id in Y||"line"!==c||(d=!0)});return d}function mb(a){a="string"===typeof a?a:a.id;return!(a in Y)||"line"===Y[a]||"spline"===Y[a]}function Aa(a){return"bar"===
...
function(a,c){tb?tb=!1:g.selectAll(".-shape-"+c).each(function(a){var d=d3.select(this),b=d.classed(ba),f=!1,e;"circle"===this.nodeName?(f=Ub(this,1.5*Ga),e=Xb):"rect"===this.nodeName&&(f=Vb(this),e=Zb);if(ka||f)$&&la(a)&&(d.classed(ba,!b),e(!b,d,a,c)),Qc(a,d)})}).call(d3.behavior.drag().origin(Object).on("drag",function(a){if($&&(!ja||F.altDomain)){a=ub[0];var c=ub[1],d=d3.mouse(this),b=d[0],d=d[1],e=Math.min(a,b),p=Math.max(a,b),k=ka?v:Math.min(c,d),h=ka?s:Math.max(c,d);g.select(".dragarea").attr("x",
e).attr("y",k).attr("width",p-e).attr("height",h-k);g.selectAll(".-shapes").selectAll(".-shape").filter(function(a){return la(a)}).each(function(a,c){var d=d3.select(this),b=d.classed(ba),f=d.classed(vb),g,n,m,l;g=!1;"circle"===this.nodeName?(g=1*d.attr("cx"),n=1*d.attr("cy"),l=Xb,g=e<g&&g<p&&k<n&&n<h):"rect"===this.nodeName&&(g=1*d.attr("x"),n=1*d.attr("y"),m=1*d.attr("width"),l=Zb,g=e<g+m&&g<p&&n<h);g^f&&(d.classed(vb,!f),d.classed(ba,!b),l(!b,d,a,c))})}}).on("dragstart",function(){$&&(ub=d3.mouse(this),
g.select(".chart").append("rect").attr("class","dragarea").style("opacity",0.1),Za=!0)}).on("dragend",function(){$&&(g.select(".dragarea").transition().duration(100).style("opacity",0).remove(),g.selectAll(".-shape").classed(vb,!1),Za=!1)})).call(F).on("dblclick.zoom",null);g.select(".chart").append("g").attr("class","chart-bars");g.select(".chart").append("g").attr("class","chart-lines");if(ja)g.insert("rect",Rc?null:"g.grid").attr("class","zoom-rect").attr("width",t).attr("height",s).style("opacity",
...
a.selectAll("text").attr("x",e?t:0).attr("y",Fa));c&&ac&&(ygrid=g.select(".ygrids").selectAll(".ygrid").data(x.ticks(10)),ygrid.enter().append("line").attr("class","ygrid"),ygrid.attr("x1",e?x:0).attr("x2",e?x:t).attr("y1",e?0:x).attr("y2",e?s:x).attr("opacity",0).transition().attr("opacity",1),ygrid.exit().remove());c&&rb&&g.select(".ygrid-lines").selectAll(".ygrid-line").attr("y1",Pb).attr("y2",Pb);h=ha?1.2*M.tickOffset()/b:0.6*((e?s:t)*T(!1)/(Ea()-1));n=Sb(e?null:s);a=Qb(h,b,d);f=Rb(n,d,e);c=g.selectAll(".-bars").selectAll(".-bar").data(Tb);
c.transition().duration(q).attr("x",e?f:a).attr("y",e?a:f).attr("width",e?n:h).attr("height",e?h:n);c.enter().append("rect").attr("class",Ob).attr("x",e?f:a).attr("y",e?a:f).attr("width",e?n:h).attr("height",e?h:n).style("opacity",0).transition().duration(q).style("opacity",1);c.exit().transition().duration(q).style("opacity",0).remove();g.selectAll(".-line").transition().duration(q).attr("d",Uc);c=g.selectAll(".-circles").selectAll(".-circle").data(Cc);c.transition().duration(q).attr("cx",e?X:W).attr("cy",
e?W:X);c.enter().append("circle").attr("class",zc).attr("cx",e?X:W).attr("cy",e?W:X).attr("r",sb);c.exit().remove();qa&&(null!==d3.event&&"zoom"===d3.event.type&&E.extent(m.orgDomain()).update(),u&&(w.select(".x.axis").transition().duration(e?q:0).call(e?ga:za),E.empty()||E.extent(m.orgDomain()).update(),h=ha?1.2*za.tickOffset()/b:0.6*((e?s:t)*T(!0)/(Ea()-1)),n=Sb(fa,!0),a=Qb(h,b,d,!0),f=Rb(n,d,!1,!0),contextBar=w.selectAll(".-bars").selectAll(".-bar").data(Tb),contextBar.transition().duration(q).attr("x",
a).attr("y",f).attr("width",h).attr("height",n),contextBar.enter().append("rect").attr("class",Ob).attr("x",a).attr("y",f).attr("width",h).attr("height",n).style("opacity",0).transition().style("opacity",1),contextBar.exit().transition().style("opacity",0).remove(),w.selectAll(".-line").transition().duration(q).attr("d",Vc)));g.selectAll(".selected-circles").filter(function(a){return Aa(a)}).selectAll("circle").remove();g.selectAll(".selected-circle").transition().duration(q).attr("cx",e?X:W).attr("cy",
...
[d3.time.format(":%S"),function(a){return a.getSeconds()}],[d3.time.format(".%L"),function(a){return a.getMilliseconds()}]];return function(c){for(var b=a.length-1,e=a[b];!e[1](c);)e=a[--b];return e[0](c)}}(),Cb,Hb,Eb,Pa,Gb,Ib,t,s,fa,bb,pa,Jb,Kb,eb,fb,m,x,O,V,Qa,Ra,M,ga,Sa,za,kc=e?"left":"bottom",lc=e?db?"top":"bottom":db?"right":"left",mc=e?cb?"bottom":"top":cb?"left":"right",nc="bottom",I={main:function(){return"translate("+H+","+v+")"},context:function(){return"translate("+G+","+ua+")"},legend:function(){return"translate("+
Oa+","+xb+")"},y2:function(){return"translate("+(e?0:t)+","+(e?10:0)+")"},x:function(){return"translate(0,"+s+")"},subx:function(){return"translate(0,"+fa+")"}},Uc=function(){var a=d3.svg.line().x(e?function(a){return h(a.id)(a.value)}:Xa).y(e?Xa:function(a){return h(a.id)(a.value)});return function(c){var b;if(mb(c))return"spline"===Y["string"===typeof c?c:c.id]?a.interpolate("cardinal"):a.interpolate("linear"),0<Object.keys(hc).length?Hc(c.values,m,h(c.id),hc[c.id]):a(c.values);b=m(c.values[0].x);
c=h(c.id)(c.values[0].value);return e?"M "+c+" "+b:"M "+b+" "+c}}(),Vc=function(){var a=d3.svg.line().x(function(a){return V(a.x)}).y(function(a){return Na(a.id)(a.value)});return function(c){return mb(c)?a(c.values):"M "+V(c.values[0].x)+" "+Na(c.id)(c.values[0].value)}}(),E=d3.svg.brush().on("brush",function(){z({withTransition:!1,withY:!1,withSubchart:!1,withUpdateXDomain:!0})}),F=d3.behavior.zoom().on("zoomstart",function(){F.altDomain=d3.event.sourceEvent.altKey?m.orgDomain():null}).on("zoom",
ja?Wc:null);E.update=function(){w&&w.select(".x.brush").call(this);return this};F.orgScaleExtent=function(){var a=gc?gc:[1,10];return[a[0],Math.max(Ea()/a[1],a[1])]};F.updateScaleExtent=function(){var a=m.orgDomain(),a=(a[1]-a[0])/(P[1]-P[0]),c=this.orgScaleExtent();this.scaleExtent([c[0]*a,c[1]*a]);return this};var Ha,Ya,g,w,ra,Z,Wa=null,hb=null,P;k.focus=function(a){k.defocus();d3.selectAll(Ma(a)).filter(function(a){return jb(a.id)}).classed("focused",!0).transition().duration(100).style("opacity",
1)};k.defocus=function(a){d3.selectAll(Ma(a)).filter(function(a){return jb(a.id)}).classed("focused",!1).transition().duration(100).style("opacity",0.3)};k.revert=function(a){d3.selectAll(Ma(a)).filter(function(a){return jb(a.id)}).classed("focused",!1).transition().duration(100).style("opacity",1)};k.show=function(a){d3.selectAll(Ma(a)).transition().style("opacity",1)};k.hide=function(a){d3.selectAll(Ma(a)).transition().style("opacity",0)};k.unzoom=function(){E.clear().update();z({withUpdateXDomain:!0})};
k.load=function(a){u(a.done)&&(a.done=function(){});if("cacheIds"in a&&K(a.cacheIds))Ka(ab(a.cacheIds),a.done);else if("data"in a)Ka(da(data),a.done);else if("url"in a)d3.csv(a.url,function(c,b){Ka(da(b),a.done)});else if("rows"in a)Ka(da(wa(a.rows)),a.done);else if("columns"in a)Ka(da(xa(a.columns)),a.done);else throw Error("url or rows or columns is required.");};k.unload=function(a){k.data.targets=k.data.targets.filter(function(c){return c.id!=a});d3.selectAll(".target-"+a).transition().style("opacity",
0).remove();sa&&(d3.selectAll(".legend-item-"+a).remove(),$a(k.data.targets));0<k.data.targets.length&&z()};k.selected=function(a){a=l(a)?"-"+a:"";return d3.merge(g.selectAll(".-shapes"+a).selectAll(".-shape").filter(function(){return d3.select(this).classed(ba)}).map(function(a){return a.map(function(a){return a.__data__})}))};k.select=function(a,c,b){$&&g.selectAll(".-shapes").selectAll(".-shape").each(function(e,f){var g="circle"===this.nodeName?Wb:Gc,h="circle"===this.nodeName?nb:Yb;0<=c.indexOf(f)?
la(e)&&(ka||u(a)||0<=a.indexOf(e.id))&&g(d3.select(this).classed(ba,!0),e,f):l(b)&&b&&h(d3.select(this).classed(ba,!1),e,f)})};k.unselect=function(a,b){$&&g.selectAll(".-shapes").selectAll(".-shape").each(function(d,e){var f="circle"===this.nodeName?nb:Yb;(u(b)||0<=b.indexOf(e))&&la(d)&&(ka||u(a)||0<=a.indexOf(d.id))&&f(d3.select(this).classed(ba,!1),d,e)})};k.toLine=function(a){lb(a,"line");z()};k.toSpline=function(a){lb(a,"spline");z()};k.toBar=function(a){lb(a,"bar");z()};k.groups=function(a){if(u(a))return q;
q=a;z();return q};k.regions=function(a){if(u(a))return J;J=a;z();return J};k.regions.add=function(a){if(u(a))return J;J=J.concat(a);z();return J};k.regions.remove=function(a,b){var d=[].concat(a);b=l(b)?b:{};d.forEach(function(a){var d=d3.selectAll("."+a);l(b.duration)&&(d=d.transition().duration(b.duration).style("fill-opacity",0));d.remove();J=J.filter(function(b){return 0>b.classes.indexOf(a)})});return J};k.data.get=function(a){a=k.data.getAsTarget(a);return l(a)?a.values.map(function(a){return a.value}):
void 0};k.data.getAsTarget=function(a){var b=Ta(function(b){return b.id==a});return 0<b.length?b[0]:void 0};if("url"in R.data)d3.csv(R.data.url,function(a,b){ob(b)});else if("rows"in R.data)ob(wa(R.data.rows));else if("columns"in R.data)ob(xa(R.data.columns));else throw Error("url or rows or columns is required.");wb.onresize=function(){N();S();E.x(V);ja&&F.x(m);d3.select("svg").attr("width",bb).attr("height",pa);d3.select("#"+pb).select("rect").attr("width",t).attr("height",s);d3.select("#xaxis-clip").select("rect").attr("width",
t+2);d3.select(".zoom-rect").attr("width",t).attr("height",s);g.select(".x.axis").attr("transform",I.x);g.select(".y2.axis").attr("transform",I.y2);qa&&(w.select(".x.brush").selectAll("rect").attr("height",fa),w.attr("transform",I.context),w.select(".x.axis").attr("transform",I.subx));sa&&(ra.attr("transform",I.legend),$a(k.data.targets,{withTransition:!1}));z({withTransition:!1,withUpdateXDomain:!0})};return k}})(window);
