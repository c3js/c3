(function(xb){function jc(){function l(b,h){b.attr("transform",function(b){return"translate("+(h(b)+R)+",0)"})}function s(b,h){b.attr("transform",function(b){return"translate(0,"+h(b)+")"})}function Oa(b){var h=b[0];b=b[b.length-1];return h<b?[h,b]:[b,h]}function ta(b){for(var h=[],l=Math.ceil(b[0]);l<b[1];l++)h.push(l);0<h.length&&0<h[0]&&h.unshift(h[0]-(h[1]-h[0]));return h}function q(z){z.each(function(){var h=d3.select(this),z=ta(G.domain()),q=h.selectAll(".tick.major").data(z,String),B=q.enter().insert("g",
"path").attr("class","tick major").style("opacity",1E-6),ja=d3.transition(q.exit()).style("opacity",1E-6).remove(),ka=d3.transition(q).style("opacity",1),C,S=G.rangeExtent?G.rangeExtent():Oa(G.range()),h=h.selectAll(".domain").data([0]),h=(h.enter().append("path").attr("class","domain"),d3.transition(h)),K=G.copy(),bb=this.__chart__||K;this.__chart__=K;B.append("line");B.append("text");var la=B.select("line"),ua=ka.select("line"),ba=q.select("text"),va=B.select("text"),wa=ka.select("text");R=(K(1)-
K(0))/2;q=b?0:R;switch(v){case "bottom":C=l;la.attr("y2",D);va.attr("y",Math.max(D,0)+H);ua.attr("x1",q).attr("x2",q).attr("y2",D);wa.attr("x",0).attr("y",Math.max(D,0)+H);ba.attr("dy",".71em").style("text-anchor","middle");ba.text(function(b){return z.length<N||0==b%Math.ceil(z.length/N)?b<L.length?L[b]:b:""});h.attr("d","M"+S[0]+","+T+"V0H"+S[1]+"V"+T);break;case "left":C=s,la.attr("x2",-D),va.attr("x",-(Math.max(D,0)+H)),ua.attr("x2",-D).attr("y2",0),wa.attr("x",-(Math.max(D,0)+H)).attr("y",R),
ba.attr("dy",".32em").style("text-anchor","end"),ba.text(function(b){return z.length<N||0==b%Math.ceil(z.length/N)?b<L.length?L[b]:b:""}),h.attr("d","M"+-T+","+S[0]+"H0V"+S[1]+"H"+-T)}if(G.ticks)B.call(C,bb),ka.call(C,K),ja.call(C,K);else{var ca=K.rangeBand()/2,ja=function(b){return K(b)+ca};B.call(C,ja);ka.call(C,ja)}})}var G=d3.scale.linear(),v="bottom",D=6,T=6,H=3,b=!1,N=10,R=0,L=[];q.scale=function(b){if(!arguments.length)return G;G=b;return q};q.orient=function(b){if(!arguments.length)return v;
v=b in{top:1,right:1,bottom:1,left:1}?b+"":"bottom";return q};q.categories=function(b){if(!arguments.length)return L;L=b;return q};q.tickCentered=function(l){if(!arguments.length)return b;b=l;return q};q.tickTextNum=function(b){if(!arguments.length)return N;N=b;return q};q.tickOffset=function(){return R};q.ticks=function(){};return q}function s(l){return"undefined"===typeof l}function l(l){return"undefined"!==typeof l}xb.c3={};c3.generate=function(Q){var yb,Oa,ta,q,G,v,D,T,H;function b(a,c){for(var d=
Q,m=0;m<a.length;m++){if(!(a[m]in d))return c;d=d[a[m]]}return d}function N(){cb=null===Bb?+d3.select(xa).style("width").replace("px",""):Bb;var a=null===Cb?+d3.select(xa).style("height").replace("px",""):Cb;ma=0<a?a:320;Db=20+Eb+da;Fb=!Gb||e||db?1:50;Pa=eb?0:40;Hb=ma-Eb-da;Ib=20+da;Jb=ma-da;v=0;D=Fb;T=Db;H=Pa;ta=Hb;q=Ib;G=Pa;yb=Jb;Oa=Pa;n=cb-H-D;t=ma-v-T;ea=ma-ta-q}function R(){Kb=e?10:0;Lb=e?t:n;fb=e?0:t;gb=e?n:1;r=L(Kb,Lb,l(r)?r.domain():void 0,function(a){return M.tickOffset()});y=z(fb,gb,l(y)?
y.domain():void 0);O=z(fb,gb,l(O)?O.domain():void 0);U=L(0,n,l(ya)?ya:void 0,function(a){return 0===a%1?za.tickOffset():0});Qa=z(ea,10);Ra=z(ea,10);M=zb(r,kc);fa=d3.svg.axis().scale(y).orient(lc);Sa=d3.svg.axis().scale(O).orient(mc);za=zb(U,nc)}function L(a,c,d,m){var f=(E?d3.time.scale():d3.scale.linear()).range([a,c]);f.orgDomain=function(){return f.domain()};l(d)&&f.domain(d);s(m)&&(m=function(a){return 0});if(na){var b=f,p,f=function(a){return b(a)+m(a)};for(p in b)f[p]=b[p];f.orgDomain=function(){return b.domain()};
f.domain=function(a){if(!arguments.length)return a=b.domain(),[a[0],a[1]+1];b.domain(a);return f}}return f}function z(a,c){return d3.scale.linear().range([a,c])}function h(a){return"y2"===B(a)?O:y}function Na(a){return"y2"===B(a)?Ra:Qa}function zb(a,c){var d=(na?jc():d3.svg.axis()).scale(a).orient(c);E&&d.tickFormat(oc);na?d.categories(hb).tickCentered(pc):d.tickOffset=function(){return 0};return d}function B(a){return a in Mb?Mb[a]:"y"}function ja(a){return d3.min(a,function(a){return d3.min(a.values,
function(a){return a.value})})}function ka(a){var c={},d,m;a.forEach(function(a){c[a.id]=[];a.values.forEach(function(d){c[a.id].push(d.value)})});for(d=0;d<w.length;d++)for(m=1;m<w[d].length;m++)Aa(w[d][m])&&(s(c[w[d][m]])||c[w[d][m]].forEach(function(a,b){B(w[d][m])===B(w[d][0])&&(c[w[d][0]][b]+=1*a)}));return d3.max(Object.keys(c).map(function(a){return d3.max(c[a])}))}function C(a,c){var d=Ta(function(a){return B(a.id)===c}),m="y2"===c?qc:rc,f="y2"===c?sc:tc,m=null!==m?m:ja(d),f=null!==f?f:ka(d),
b=0.1*Math.abs(f-m),p=b,e=b,g="y2"===c?uc:vc;null!==g&&(yDomainAbs=Math.max(Math.abs(m),Math.abs(f)),f=yDomainAbs-g,m=g-yDomainAbs);"y"===c&&null!==Ba&&(p=l(Ba.top)?Ba.top:b,e=l(Ba.bottom)?Ba.bottom:b);"y2"===c&&null!==Ca&&(p=l(Ca.top)?Ca.top:b,e=l(Ca.bottom)?Ca.bottom:b);return[wc(d,"bar")?0:m-e,f+p]}function S(a){var c;a?(a=r.domain(),c=F.extent()):(a=ya,c=r.domain());return(a[1]-a[0])/(c[1]-c[0])}function K(a){for(var c=0;c<a.length;c++)if(!(a[c]in Ua))return!1;return!0}function bb(a){for(var c=
[],d=0;d<a.length;d++)a[d]in Ua&&c.push(Ab(Ua[a[d]]));return c}function la(a){return"start"in a?r(E?ga(a.start):a.start):0}function ua(a){var c=la(a);a=("end"in a?r(E?ga(a.end):a.end):n)-c;return 0>a?0:a}function ba(a){var c=Da[a.id];a.name=l(c)?c:a.id;return a}function va(a){var c=a[0],d={},m=[],f,b;for(f=1;f<a.length;f++){d={};for(b=0;b<a[f].length;b++)d[c[b]]=a[f][b];m.push(d)}return m}function wa(a){var c=[],d,b,f;for(d=0;d<a.length;d++)for(f=a[d][0],b=1;b<a[d].length;b++)s(c[b-1])&&(c[b-1]={}),
c[b-1][f]=a[d][b];return c}function ca(a){var c=d3.keys(a[0]).filter(function(a){return a!==Va}),d=0,b;a.forEach(function(a){if(E){if(!(Va in a))throw Error("'"+Va+"' must be included in data");b=ga(a[Va]);if(null===b)throw Error("Failed to parse timeseries date in data");a.x=b}else a.x=d++;null===Wa&&(Wa=new Date(a.x));ib=new Date(a.x)});c=c.map(function(c,d){var b=xc(c);return{id:b,id_org:c,values:a.map(function(a){return{x:a.x,value:+a[c],id:b}})}});c.forEach(function(a){Ua[a.id_org]=Ab(a)});return c}
...
d,b)}function Gc(a,c,d){}function Zb(a,c,d){}function $b(a,c,d,b){}function Hc(a,c,d,b){var f,g,p="M",k,h,u,r,q,n=[];if(l(b))for(f=0;f<b.length;f++)n[f]={},s(b[f].start)?n[f].start=a[0].x:E&&(n[f].start=ga(b[f].start)),s(b[f].end)?n[f].end=a[a.length-1].x:E&&(n[f].end=ga(b[f].end));r=e?function(a){return d(a.value)}:function(a){return c(a.x)};q=e?function(a){return c(a.x)}:function(a){return d(a.value)};b=E?function(a,b,f,e){var p=a.x.getTime();a=b.x-a.x;b=new Date(p+a*(f+e));return"M"+c(new Date(p+
...
y.domain(C(c,"y"));O.domain(C(c,"y2"));U.domain(r.domain());Qa.domain(y.domain());Ra.domain(O.domain());M.ticks(10>a.length?a.length:10);fa.ticks(Ic).outerTickSize(0).tickFormat(Jc);Sa.ticks(Kc).outerTickSize(0).tickFormat(Lc);ya=r.domain();F.x(U);oa&&Ga.x(r);Ha=d3.select(xa).append("svg").attr("width",n+H+D).attr("height",t+v+T);Za=Ha.append("defs");Za.append("clipPath").attr("id",qb).append("rect").attr("y",v).attr("width",n).attr("height",t-v);Za.append("clipPath").attr("id","xaxis-clip").append("rect").attr("x",
...
d.classed(aa,!b),n(!b,d,a,c))})}}).on("dragstart",function(){Z&&(vb=d3.mouse(this),g.select(".chart").append("rect").attr("class","dragarea").style("opacity",0.1),$a=!0)}).on("dragend",function(){Z&&(g.select(".dragarea").transition().duration(100).style("opacity",0).remove(),g.selectAll(".-shape").classed(wb,!1),$a=!1)})).call(Ga).on("dblclick.zoom",null);g.select(".chart").append("g").attr("class","chart-bars");g.select(".chart").append("g").attr("class","chart-lines");null!==Ja&&F.extent("function"!==
typeof Ja?Ja:E?Ja(Wa,ib):Ja(0,Xa()-1));pa&&(x.append("g").attr("clip-path",Ia).attr("class","chart"),x.select(".chart").append("g").attr("class","chart-bars"),x.select(".chart").append("g").attr("class","chart-lines"),x.append("g").attr("clip-path",Ia).attr("class","x brush").call(F).selectAll("rect").attr("height",ea),x.append("g").attr("class","x axis").attr("transform",I.subx).call(za));ra&&ab(c);fc(c);A({withTransition:!1,withUpdateXDomain:!0});if(Pc){if(E&&"string"==typeof sa){sa=ga(sa);for(a=
0;a<c[0].values.length&&0!=c[0].values[a].x-sa;a++);sa=a}Y.html(ec(c.map(function(a){return ba(a.values[sa])})));Y.style("top",gc.top).style("left",gc.left).style("visibility","visible")}}function A(a){var c,d=Bc(),b=d.__max__+1,f,h,p,q,s,u;a=l(a)?a:{};c=l(a.withY)?a.withY:!0;s=l(a.withSubchart)?a.withSubchart:!0;u=l(a.withTransition)?a.withTransition:!0;a=l(a.withUpdateXDomain)?a.withUpdateXDomain:!1;u=u?250:0;a&&(r.domain(F.empty()?ya:F.extent()),oa&&Ga.x(r));y.domain(C(k.data.targets,"y"));O.domain(C(k.data.targets,
...
ra&&ab(a);d3.selectAll(".target").transition().style("opacity",1)}function Ka(a,c){k.data.targets.forEach(function(c){for(var b=0;b<a.length;b++)if(c.id===a[b].id){c.values=a[b].values;a.splice(b,1);break}});k.data.targets=k.data.targets.concat(a);fc(k.data.targets);A();c()}function ab(a,c){var b=jb(a),e,f=n/2-La*Object.keys(a).length/2,g;c=s(c)?{}:c;g=l(c.withTransition)?c.withTransition:!0;e=qa.selectAll(".legend-item").data(b).enter().append("g").attr("class",function(a){return"legend-item legend-item-"+
a}).style("cursor","pointer").on("click",function(a){Uc(a)}).on("mouseover",function(a){d3.selectAll(".legend-item").filter(function(c){return c!==a}).transition().duration(100).style("opacity",0.3);k.focus(a)}).on("mouseout",function(a){d3.selectAll(".legend-item").transition().duration(100).style("opacity",1);k.revert()});e.append("rect").attr("class","legend-item-event").style("fill-opacity",0).attr("x",-200).attr("y",function(){return da/2-16}).attr("width",La).attr("height",24);e.append("rect").attr("class",
"legend-item-tile").style("fill",function(a){return P(a)}).attr("x",-200).attr("y",function(){return da/2-9}).attr("width",10).attr("height",10);e.append("text").text(function(a){return l(Da[a])?Da[a]:a}).attr("x",-200).attr("y",function(a,c){return da/2});qa.selectAll("rect.legend-item-event").data(b).transition().duration(g?250:0).attr("x",function(a,c){return f+La*c});qa.selectAll("rect.legend-item-tile").data(b).transition().duration(g?250:0).attr("x",function(a,c){return f+La*c});qa.selectAll("text").data(b).transition().duration(g?
250:0).attr("x",function(a,c){return f+La*c+14})}function Ma(a){return l(a)?".target-"+a:".target"}var k={data:{}},Ua={},$="_expanded_",aa="_selected_",wb="_included_",xa=b(["bindto"],"#chart"),Bb=b(["size","width"],null),Cb=b(["size","height"],null),oa=b(["zoom","enabled"],!1);if(!("data"in Q))throw Error("data is required in config");var Va=b(["data","x"],"x"),Vc=b(["data","x_format"],"%Y-%m-%d"),xc=b(["data","id_converter"],function(a){return a}),Da=b(["data","names"],{}),w=b(["data","groups"],
[]),Mb=b(["data","axes"],{}),X=b(["data","types"],{}),hc=b(["data","regions"],{}),Wc=b(["data","colors"],{}),Z=b(["data","selection","enabled"],!1),ha=b(["data","selection","grouped"],!1),ia=b(["data","selection","isselectable"],function(a){return!0}),pa=b(["subchart","show"],!1),Eb=pa?b(["subchart","size","height"],60):0,Xc=b(["color","pattern"],null),ra=b(["legend","show"],!0),La=b(["legend","item","width"],80),Uc=b(["legend","item","onclick"],function(){}),ic=b(["axis","x","type"],"indexed"),hb=
b(["axis","x","categories"],[]),pc=b(["axis","x","tick","centered"],!1),Ja=b(["axis","x","default"],null),tc=b(["axis","y","max"],null),rc=b(["axis","y","min"],null),vc=b(["axis","y","center"],null),Mc=b(["axis","y","text"],null);b(["axis","y","rescale"],!0);var eb=b(["axis","y","inner"],!1),Jc=b(["axis","y","format"],function(a){return a}),Ba=b(["axis","y","padding"],null),Ic=b(["axis","y","ticks"],10),Gb=b(["axis","y2","show"],!1),sc=b(["axis","y2","max"],null),qc=b(["axis","y2","min"],null),uc=
b(["axis","y2","center"],null);b(["axis","y2","text"],null);b(["axis","y2","rescale"],!0);var db=b(["axis","y2","inner"],!1),Lc=b(["axis","y2","format"],function(a){return a}),Ca=b(["axis","y2","padding"],null),Kc=b(["axis","y2","ticks"],10),e=b(["axis","rotated"],!1),ac=b(["grid","x","show"],!1),Qc=b(["grid","x","type"],"tick"),rb=b(["grid","x","lines"],null),bc=b(["grid","y","show"],!1);b(["grid","y","type"],"tick");var sb=b(["grid","y","lines"],null),tb=b(["point","show"],!0)?b(["point","r"],2.5):
0,Nc=b(["point","focus","line","enabled"],!0),cc=b(["point","focus","expand","enabled"],!0),dc=b(["point","focus","expand","r"],cc?4:tb),Fa=b(["point","focus","select","r"],8),Oc=b(["point","onclick"],function(){}),Ec=b(["point","onselected"],function(){}),Fc=b(["point","onunselected"],function(){}),J=b(["regions"],[]),ec=b(["tooltip","contents"],function(a){var c="<table class='-tooltip'><tr><th colspan='2'>"+(E?a[0].x.getFullYear()+"."+(a[0].x.getMonth()+1)+"."+a[0].x.getDate():na?a[0].x<hb.length?
hb[a[0].x]:a[0].x:a[0].x)+"</th></tr>",b,e,f;for(b=0;b<a.length;b++)l(a[b])?(e=l(a[b].value)?(Math.round(100*a[b].value)/100).toFixed(2):"-",f=a[b].name):f=e="-",c+="<tr><td><span style='background-color:"+P(a[b].id)+"'></span>"+f+"</td><td class='value'>"+e+"</td></tr>";return c+"</table>"}),Pc=b(["tooltip","init","show"],!1),sa=b(["tooltip","init","x"],0),gc=b(["tooltip","init","position"],{top:"0px",left:"50px"}),qb=xa.replace("#","")+"-clip",Ia="url(#"+qb+")",E="timeseries"===ic,na="categorized"===
...
if(nb(c))return"spline"===X["string"===typeof c?c:c.id]?a.interpolate("cardinal"):a.interpolate("linear"),0<Object.keys(hc).length?Hc(c.values,r,h(c.id),hc[c.id]):a(c.values);b=r(c.values[0].x);c=h(c.id)(c.values[0].value);return e?"M "+c+" "+b:"M "+b+" "+c}}(),Sc=function(){var a=d3.svg.line().x(function(a){return U(a.x)}).y(function(a){return Na(a.id)(a.value)});return function(c){return nb(c)?a(c.values):"M "+U(c.values[0].x)+" "+Na(c.id)(c.values[0].value)}}(),F=d3.svg.brush().on("brush",function(){A({withTransition:!1,
withY:!1,withSubchart:!1,withUpdateXDomain:!0})}),Ga=d3.behavior.zoom().on("zoom",oa?Tc:null);F.update=function(){x&&x.select(".x.brush").call(this);return this};var Ha,Za,g,x,qa,Y,Wa=null,ib=null,ya;k.focus=function(a){k.defocus();d3.selectAll(Ma(a)).filter(function(a){return kb(a.id)}).classed("focused",!0).transition().duration(100).style("opacity",1)};k.defocus=function(a){d3.selectAll(Ma(a)).filter(function(a){return kb(a.id)}).classed("focused",!1).transition().duration(100).style("opacity",
0.3)};k.revert=function(a){d3.selectAll(Ma(a)).filter(function(a){return kb(a.id)}).classed("focused",!1).transition().duration(100).style("opacity",1)};k.show=function(a){d3.selectAll(Ma(a)).transition().style("opacity",1)};k.hide=function(a){d3.selectAll(Ma(a)).transition().style("opacity",0)};k.unzoom=function(){F.clear().update();A({withUpdateXDomain:!0})};k.load=function(a){s(a.done)&&(a.done=function(){});if("cacheIds"in a&&K(a.cacheIds))Ka(bb(a.cacheIds),a.done);else if("data"in a)Ka(ca(data),
a.done);else if("url"in a)d3.csv(a.url,function(c,b){Ka(ca(b),a.done)});else if("rows"in a)Ka(ca(va(a.rows)),a.done);else if("columns"in a)Ka(ca(wa(a.columns)),a.done);else throw Error("url or rows or columns is required.");};k.unload=function(a){k.data.targets=k.data.targets.filter(function(c){return c.id!=a});d3.selectAll(".target-"+a).transition().style("opacity",0).remove();ra&&(d3.selectAll(".legend-item-"+a).remove(),ab(k.data.targets));0<k.data.targets.length&&A()};k.selected=function(a){a=
l(a)?"-"+a:"";return d3.merge(g.selectAll(".-shapes"+a).selectAll(".-shape").filter(function(){return d3.select(this).classed(aa)}).map(function(a){return a.map(function(a){return a.__data__})}))};k.select=function(a,c,b){Z&&g.selectAll(".-shapes").selectAll(".-shape").each(function(e,f){var g="circle"===this.nodeName?Xb:Gc,h="circle"===this.nodeName?ob:Zb;0<=c.indexOf(f)?ia(e)&&(ha||s(a)||0<=a.indexOf(e.id))&&g(d3.select(this).classed(aa,!0),e,f):l(b)&&b&&h(d3.select(this).classed(aa,!1),e,f)})};
k.unselect=function(a,b){Z&&g.selectAll(".-shapes").selectAll(".-shape").each(function(d,e){var f="circle"===this.nodeName?ob:Zb;(s(b)||0<=b.indexOf(e))&&ia(d)&&(ha||s(a)||0<=a.indexOf(d.id))&&f(d3.select(this).classed(aa,!1),d,e)})};k.toLine=function(a){mb(a,"line");A()};k.toSpline=function(a){mb(a,"spline");A()};k.toBar=function(a){mb(a,"bar");A()};k.groups=function(a){if(s(a))return w;w=a;A();return w};k.regions=function(a){if(s(a))return J;J=a;A();return J};k.regions.add=function(a){if(s(a))return J;
J=J.concat(a);A();return J};k.regions.remove=function(a,b){var d=[].concat(a);b=l(b)?b:{};d.forEach(function(a){var d=d3.selectAll("."+a);l(b.duration)&&(d=d.transition().duration(b.duration).style("fill-opacity",0));d.remove();J=J.filter(function(b){return 0>b.classes.indexOf(a)})});return J};k.data.get=function(a){a=k.data.getAsTarget(a);return l(a)?a.values.map(function(a){return a.value}):void 0};k.data.getAsTarget=function(a){var b=Ta(function(b){return b.id==a});return 0<b.length?b[0]:void 0};
if("url"in Q.data)d3.csv(Q.data.url,function(a,b){pb(b)});else if("rows"in Q.data)pb(va(Q.data.rows));else if("columns"in Q.data)pb(wa(Q.data.columns));else throw Error("url or rows or columns is required.");xb.onresize=function(){N();R();F.x(U);oa&&Ga.x(r);d3.select("svg").attr("width",cb).attr("height",ma);d3.select("#"+qb).select("rect").attr("width",n).attr("height",t);d3.select("#xaxis-clip").select("rect").attr("width",n+2);d3.select(".zoom-rect").attr("width",n).attr("height",t);g.select(".x.axis").attr("transform",
I.x);g.select(".y2.axis").attr("transform",I.y2);pa&&(x.select(".x.brush").selectAll("rect").attr("height",ea),x.attr("transform",I.context),x.select(".x.axis").attr("transform",I.subx));ra&&(qa.attr("transform",I.legend),ab(k.data.targets,{withTransition:!1}));A({withTransition:!1,withUpdateXDomain:!0})};return k}})(window);
