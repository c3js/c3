...
Q=m.selectAll(".tick.major").data(ja,String),R=Q.enter().insert("g","path").attr("class","tick major").style("opacity",1E-6),S=d.transition(Q.exit()).style("opacity",1E-6).remove(),x=d.transition(Q).style("opacity",1),L,K=u.rangeExtent?u.rangeExtent():N(u.range()),m=m.selectAll(".domain").data([0]);m.enter().append("path").attr("class","domain");var m=d.transition(m),C=u.copy(),ba=this.__chart__||C;this.__chart__=C;R.append("line");R.append("text");var ka=R.select("line"),Wa=x.select("line"),la=Q.select("text"),
...
"visible").data([a]).attr(h?"y1":"x1",xa).attr(h?"y2":"x2",xa)}function Hc(){k.select("line.xgrid-focus").style("visibility","hidden")}function ea(a){return a.x||0===a.x?n(a.x):null}function fa(a){return x(a.id)(a.value)}function Nd(){var a={},b=0,c,f;ib(Ha).forEach(function(l){for(c=0;c<v.length;c++)if(!(0>v[c].indexOf(l.id)))for(f=0;f<v[c].length;f++)if(v[c][f]in a){a[l.id]=a[v[c][f]];break}t(a[l.id])&&(a[l.id]=b++)});a.__max__=b-1;return a}function Ic(a,b,c,f){var l=f?T:n;return b?function(f){var d=
