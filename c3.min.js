...
[],d=0;d<a.length;d++)a[d]in $a&&b.push(Db($a[a[d]]));return b}function ta(a){return"start"in a?l(A?la(a.start):a.start):0}function Da(a){var b=ta(a);a=("end"in a?l(A?la(a.end):a.end):q)-b;return 0>a?0:a}function ga(a){var b=La[a.id];a.name=n(b)?b:a.id;return a}function Ea(a){var b=a[0],d={},h=[],e,c;for(e=1;e<a.length;e++){d={};for(c=0;c<a[e].length;c++)d[b[c]]=a[e][c];h.push(d)}return h}function Fa(a){var b=[],d,h,e;for(d=0;d<a.length;d++)for(e=a[d][0],h=1;h<a[d].length;h++)s(b[h-1])&&(b[h-1]={}),
b[h-1][e]=a[d][h];return b}function ha(a){var b=e.keys(a[0]).filter(function(a){return a!==ma}),d=0,h;if(A&&!ma)return alert('data.x must be specified when axis.x.type == "timeseries"'),[];a.forEach(function(a){if(A){if(!(ma in a))throw Error("'"+ma+"' must be included in data");h=la(a[ma]);if(null===h)throw Error("Failed to parse timeseries date in data");a.x=h}else a.x=Rb?a[ma]:d++;null===ab&&(ab=new Date(a.x));mb=new Date(a.x)});b=b.map(function(b){var d=Gc(b);return{id:d,id_org:b,values:a.map(function(a){return{x:a.x,
...
function pb(a){return"-shapes -shapes-"+a.id}function Sb(a){return pb(a)+" -line -line-"+a.id}function gb(a){return pb(a)+" -circles -circles-"+a.id}function Tb(a){return pb(a)+" -bars -bars-"+a.id}function Hc(a,b){return"-shape -shape-"+b+" -circle -circle-"+b}function Ub(a,b){return"-shape -shape-"+b+" -bar -bar-"+b}function Ic(a,b){return"region region-"+b+" "+("classes"in a?[].concat(a.classes).join(" "):"")}function bb(a){return l(a.x)}function Ma(a){return l(A?la(a.value):a.value)}function Vb(a){return z(a.value)}
function Z(a){return l(a.x)}function $(a){return fa(a.id)(a.value)}function Jc(){var a={},b=0,d,h;Za(Ia).forEach(function(e){for(d=0;d<v.length;d++)if(!(0>v[d].indexOf(e.id)))for(h=0;h<v[d].length;h++)if(v[d][h]in a){a[e.id]=a[v[d][h]];break}s(a[e.id])&&(a[e.id]=b++)});a.__max__=b-1;return a}function Wb(a,b,d,e){var p=e?X:l;return function(e){var h=e.id in d?d[e.id]:0;return p(e.x)-a*(b/2-h)}}function Xb(a,b,d,e){var p=Object.keys(b);return function(c,f){var g=0,k=e?Ua(c.id):fa(c.id);Za(Ia).forEach(function(d){d.id!==
c.id&&b[d.id]===b[c.id]&&p.indexOf(d.id)<p.indexOf(c.id)&&(g+=a(d.values[f]))});return d?g:k(c.value)-g}}function Yb(a,b){var d=null===a?function(a){return a}:function(b){return a-b};return function(a){var e=b?Ua(a.id):fa(a.id);return d(e(a.value))}}function qb(a,b){var d=s(a)?nb():a;"string"===typeof d&&(d=[d]);for(var e=0;e<d.length;e++)aa[d[e]]=b}function Fc(a,b){var d=!1;a.forEach(function(a){aa[a.id]===b&&(d=!0);a.id in aa||"line"!==b||(d=!0)});return d}function rb(a){a="string"===typeof a?a:
...
0).remove()}function cc(a,b,d,e){a?bc(b,d,e):sb(b,d,e)}function Oc(){}function dc(){}function ec(a,b,d,e){}function fc(a){return a.filter(function(a){return null!==a.value})}function Pc(a,b,d,e){var c,g,r="M",k,l,u,gc,q,m=[];if(n(e))for(c=0;c<e.length;c++)m[c]={},s(e[c].start)?m[c].start=a[0].x:A&&(m[c].start=la(e[c].start)),s(e[c].end)?m[c].end=a[a.length-1].x:A&&(m[c].end=la(e[c].end));gc=f?function(a){return d(a.value)}:function(a){return b(a.x)};q=f?function(a){return b(a.x)}:function(a){return d(a.value)};
...
R=l.domain();F.x(X);na&&G.x(l);Oa=e.select(ua).append("svg").attr("width",q+I+E).attr("height",t+w+W).on("mouseenter",Uc).on("mouseleave",Vc);cb=Oa.append("defs");cb.append("clipPath").attr("id",ub).append("rect").attr("y",w).attr("width",q).attr("height",t-w);cb.append("clipPath").attr("id","xaxis-clip").append("rect").attr("x",-1).attr("y",-1).attr("width",q+2).attr("height",40);cb.append("clipPath").attr("id","yaxis-clip").append("rect").attr("x",-I+1).attr("y",w-1).attr("width",I).attr("height",
t-w+2);g=Oa.append("g").attr("transform",K.main);y=ya?Oa.append("g").attr("transform",K.context):null;za=Aa?Oa.append("g").attr("transform",K.legend):null;ba=e.select(ua).style("position","relative").append("div").style("position","absolute").style("width","30%").style("z-index","10").style("visibility","hidden");g.append("g").attr("class","x axis").attr("clip-path",f?"":"url(#xaxis-clip)").attr("transform",K.x).call(f?ka:J);g.append("g").attr("class","y axis").attr("clip-path",f?"url(#yaxis-clip)":
"").call(f?J:ka).append("text").attr("transform","rotate(-90)").attr("dy","1.4em").attr("dx","-.8em").style("text-anchor","end").text(Wc);Jb&&g.append("g").attr("class","y2 axis").attr("transform",K.y2).call(Ya);d=g.append("g").attr("clip-path",Pa).attr("class","grid");ic&&d.append("g").attr("class","xgrids");vb&&(c=d.append("g").attr("class","xgrid-lines").selectAll(".xgrid-line").data(vb).enter().append("g").attr("class","xgrid-line"),c.append("line").attr("class",function(a){return""+a["class"]}),
...
f=ec);if(oa||h)ca&&pa(a)&&(d.classed(ea,!c),f(!c,d,a,b)),Yc(a,d)})}).call(e.behavior.drag().origin(Object).on("drag",function(){if(ca&&(!na||G.altDomain)){var a=zb[0],b=zb[1],d=e.mouse(this),c=d[0],d=d[1],h=Math.min(a,c),f=Math.max(a,c),k=oa?w:Math.min(b,d),l=oa?t:Math.max(b,d);g.select(".dragarea").attr("x",h).attr("y",k).attr("width",f-h).attr("height",l-k);g.selectAll(".-shapes").selectAll(".-shape").filter(function(a){return pa(a)}).each(function(a,b){var d=e.select(this),c=d.classed(ea),g=d.classed(Ab),
...
g.selectAll(".-shape").classed(Ab,!1),db=!1)})).call(G).on("dblclick.zoom",null);g.select(".chart").append("g").attr("class","chart-bars");g.select(".chart").append("g").attr("class","chart-lines");if(na)g.insert("rect",Zc?null:"g.grid").attr("class","zoom-rect").attr("width",q).attr("height",t).style("opacity",0).style("cursor","ew-resize").call(G).on("dblclick.zoom",null);null!==Qa&&F.extent("function"!==typeof Qa?Qa:A?Qa(ab,mb):Qa(0,xa()-1));ya&&(y.append("g").attr("clip-path",Pa).attr("class",
"chart"),y.select(".chart").append("g").attr("class","chart-bars"),y.select(".chart").append("g").attr("class","chart-lines"),y.append("g").attr("clip-path",Pa).attr("class","x brush").call(F).selectAll("rect").attr("height",ja),y.append("g").attr("class","x axis").attr("transform",K.subx).call(Ga));Aa&&eb(b);nc(b);B({withTransition:!1,withUpdateXDomain:!0});if($c){if(A&&"string"===typeof Ba){Ba=la(Ba);for(a=0;a<b[0].values.length&&0!==b[0].values[a].x-Ba;a++);Ba=a}ba.html(mc(b.map(function(a){return ga(a.values[Ba])})));
ba.style("top",oc.top).style("left",oc.left).style("visibility","visible")}}}function B(a){var b,d,c=Jc(),p=c.__max__+1,m,r,s,v,u;a=n(a)?a:{};b=n(a.withY)?a.withY:!0;v=n(a.withSubchart)?a.withSubchart:!0;u=n(a.withTransition)?a.withTransition:!0;a=n(a.withUpdateXDomain)?a.withUpdateXDomain:!1;u=u?250:0;a&&(l.domain(F.empty()?R:F.extent()),na&&G.x(l).updateScaleExtent());z.domain(D(k.data.targets,"y"));Q.domain(D(k.data.targets,"y2"));g.select(".x.axis").transition().duration(f?u:0).call(f?ka:J);g.select(".y.axis").transition().duration(f?
0:u).call(f?J:ka);g.select(".y2.axis").transition().call(Ya);Wa.domain(z.domain());Xa.domain(Q.domain());ba.style("visibility","hidden");g.select("line.xgrid-focus").style("visibility","hidden").attr("y2",t);if(ic){if("year"===ad)for(a=[],r=ab.getFullYear(),m=mb.getFullYear();r<=m;r++)a.push(new Date(r+"-01-01 00:00:00"));else a=l.ticks(10);a=g.select(".xgrids").selectAll(".xgrid").data(a);a.enter().append("line").attr("class","xgrid");a.exit().remove();g.selectAll(".xgrid").attr("x1",function(a){return l(a)-
...
F.extent(l.orgDomain()).update(),v&&(y.select(".x.axis").transition().duration(f?u:0).call(f?ka:Ga),F.empty()||F.extent(l.orgDomain()).update(),m=Y?1.2*Ga.tickOffset()/p:0.6*((f?t:q)*V(!0)/(xa()-1)),r=Yb(ja,!0),b=Wb(m,p,c,!0),a=Xb(r,c,!1,!0),c=y.selectAll(".-bars").selectAll(".-bar").data(Zb),c.transition().duration(u).attr("x",b).attr("y",a).attr("width",m).attr("height",r),c.enter().append("rect").attr("class",Ub).attr("x",b).attr("y",a).attr("width",m).attr("height",r).style("opacity",0).transition().style("opacity",
...
0).append("g").attr("class",Tb).style("fill",function(a){return S(a.id)}).style("stroke",function(a){return S(a.id)}).style("stroke-width",0).style("cursor",function(a){return pa(a)?"pointer":null});b=g.select(".chart-lines").selectAll(".chart-line").data(a).enter().append("g").attr("class",function(a){return"chart-line target target-"+a.id}).style("pointer-events","none").style("opacity",0);b.append("path").attr("class",Sb).style("stroke",function(a){return S(a.id)});b.append("g").attr("class",function(a){return"selected-circles selected-circles-"+
a.id});b.append("g").attr("class",gb).style("fill",function(a){return S(a.id)}).style("cursor",function(a){return pa(a)?"pointer":null});a.forEach(function(a){g.selectAll(".selected-circles-"+a.id).selectAll(".selected-circle").each(function(b){b.value=a.values[b.x].value})});ya&&(b=y.select(".chart-bars").selectAll(".chart-bar").data(a),b=b.enter().append("g").attr("class",function(a){return"chart-bar target target-"+a.id}).style("opacity",0),b.append("g").attr("class",Tb).style("fill",function(a){return S(a.id)}),
...
1);k.revert()});c.append("rect").attr("class","legend-item-event").style("fill-opacity",0).attr("x",-200).attr("y",function(){return ia/2-16}).attr("width",Sa).attr("height",24);c.append("rect").attr("class","legend-item-tile").style("fill",function(a){return S(a)}).attr("x",-200).attr("y",function(){return ia/2-9}).attr("width",10).attr("height",10);c.append("text").text(function(a){return n(La[a])?La[a]:a}).attr("x",-200).attr("y",function(){return ia/2});za.selectAll("rect.legend-item-event").data(d).transition().duration(g?
250:0).attr("x",function(a,b){return f+Sa*b});za.selectAll("rect.legend-item-tile").data(d).transition().duration(g?250:0).attr("x",function(a,b){return f+Sa*b});za.selectAll("text").data(d).transition().duration(g?250:0).attr("x",function(a,b){return f+Sa*b+14})}function Ta(a){return n(a)?".target-"+a:".target"}var k={data:{}},$a={},da="_expanded_",ea="_selected_",Ab="_included_",ua=c(["bindto"],"#chart"),Eb=c(["size","width"],null),Fb=c(["size","height"],null),na=c(["zoom","enabled"],!1),pc=c(["zoom",
"extent"],null),Zc=c(["zoom","privileged"],!1),Uc=c(["onenter"],function(){}),Vc=c(["onleave"],function(){});if(!("data"in T))throw Error("data is required in config");var ma=c(["data","x"],void 0),fd=c(["data","x_format"],"%Y-%m-%d"),Gc=c(["data","id_converter"],function(a){return a}),La=c(["data","names"],{}),v=c(["data","groups"],[]),Qb=c(["data","axes"],{}),aa=c(["data","types"],{}),qc=c(["data","regions"],{}),gd=c(["data","colors"],{}),ca=c(["data","selection","enabled"],!1),oa=c(["data","selection",
...
e+"</td><td class='value'>"+c+"</td></tr>";return b+"</table>"}),$c=c(["tooltip","init","show"],!1),Ba=c(["tooltip","init","x"],0),oc=c(["tooltip","init","position"],{top:"0px",left:"50px"}),ub=ua.replace("#","")+"-clip",Pa="url(#"+ub+")",A="timeseries"===rc,Y="categorized"===rc,Rb=!A&&ma,zb=null,db=!1,yb=!1,ia=Aa?40:0,la=e.time.format(fd).parse,S=function(a,b){var d=[],c=null!==b?b:"#1f77b4 #ff7f0e #2ca02c #d62728 #9467bd #8c564b #e377c2 #7f7f7f #bcbd22 #17becf".split(" ");return function(b){if(b in
...
[e.time.format(".%L"),function(a){return a.getMilliseconds()}]];return function(b){for(var d=a.length-1,c=a[d];!c[1](b);)c=a[--d];return c[0](b)}}(),Gb,Lb,Ib,Va,Kb,Mb,q,t,ja,hb,va,Nb,Ob,kb,lb,l,z,Q,X,Wa,Xa,J,ka,Ya,Ga,tc=f?"left":"bottom",uc=f?jb?"top":"bottom":jb?"right":"left",vc=f?ib?"bottom":"top":ib?"left":"right",wc="bottom",K={main:function(){return"translate("+I+","+w+")"},context:function(){return"translate("+H+","+Ca+")"},legend:function(){return"translate("+Cb+","+Bb+")"},y2:function(){return"translate("+
(f?0:q)+","+(f?10:0)+")"},x:function(){return"translate(0,"+t+")"},subx:function(){return"translate(0,"+ja+")"}},bd=function(){var a=e.svg.line().x(f?function(a){return fa(a.id)(a.value)}:bb).y(f?bb:function(a){return fa(a.id)(a.value)});return function(b){var d=fc(b.values),c;if(rb(b))return"spline"===aa["string"===typeof b?b:b.id]?a.interpolate("cardinal"):a.interpolate("linear"),0<Object.keys(qc).length?Pc(d,l,fa(b.id),qc[b.id]):a(d);c=l(d[0].x);b=fa(b.id)(d[0].value);return f?"M "+b+" "+c:"M "+
c+" "+b}}(),cd=function(){var a=e.svg.line().x(function(a){return X(a.x)}).y(function(a){return Ua(a.id)(a.value)});return function(b){var c=fc(b.values);return rb(b)?a(c):"M "+X(c[0].x)+" "+Ua(b.id)(c[0].value)}}(),F=e.svg.brush().on("brush",function(){B({withTransition:!1,withY:!1,withSubchart:!1,withUpdateXDomain:!0})}),G=e.behavior.zoom().on("zoomstart",function(){G.altDomain=e.event.sourceEvent.altKey?l.orgDomain():null}).on("zoom",na?dd:null);F.update=function(){y&&y.select(".x.brush").call(this);
...
if("cacheIds"in a&&N(a.cacheIds))Ra(fb(a.cacheIds),a.done);else if("data"in a)Ra(ha(a.data),a.done);else if("url"in a)e.csv(a.url,function(b,c){Ra(ha(c),a.done)});else if("rows"in a)Ra(ha(Ea(a.rows)),a.done);else if("columns"in a)Ra(ha(Fa(a.columns)),a.done);else throw Error("url or rows or columns is required.");};k.unload=function(a){k.data.targets=k.data.targets.filter(function(b){return b.id!==a});e.selectAll(".target-"+a).transition().style("opacity",0).remove();Aa&&(e.selectAll(".legend-item-"+
a).remove(),eb(k.data.targets));0<k.data.targets.length&&B()};k.selected=function(a){a=n(a)?"-"+a:"";return e.merge(g.selectAll(".-shapes"+a).selectAll(".-shape").filter(function(){return e.select(this).classed(ea)}).map(function(a){return a.map(function(a){return a.__data__})}))};k.select=function(a,b,c){ca&&g.selectAll(".-shapes").selectAll(".-shape").each(function(f,g){var k="circle"===this.nodeName?bc:Oc,l="circle"===this.nodeName?sb:dc;0<=b.indexOf(g)?pa(f)&&(oa||s(a)||0<=a.indexOf(f.id))&&k(e.select(this).classed(ea,
!0),f,g):n(c)&&c&&l(e.select(this).classed(ea,!1),f,g)})};k.unselect=function(a,b){ca&&g.selectAll(".-shapes").selectAll(".-shape").each(function(c,f){var g="circle"===this.nodeName?sb:dc;(s(b)||0<=b.indexOf(f))&&pa(c)&&(oa||s(a)||0<=a.indexOf(c.id))&&g(e.select(this).classed(ea,!1),c,f)})};k.toLine=function(a){qb(a,"line");B()};k.toSpline=function(a){qb(a,"spline");B()};k.toBar=function(a){qb(a,"bar");B()};k.groups=function(a){if(s(a))return v;v=a;B();return v};k.regions=function(a){if(s(a))return L;
...
Za(function(b){return b.id===a});return 0<b.length?b[0]:void 0};if("url"in T.data)e.csv(T.data.url,function(a,b){tb(b)});else if("rows"in T.data)tb(Ea(T.data.rows));else if("columns"in T.data)tb(Fa(T.data.columns));else throw Error("url or rows or columns is required.");qa.onresize=function(){P();U();F.x(X);na&&G.x(l);e.select("svg").attr("width",hb).attr("height",va);e.select("#"+ub).select("rect").attr("width",q).attr("height",t);e.select("#xaxis-clip").select("rect").attr("width",q+2);e.select(".zoom-rect").attr("width",
q).attr("height",t);g.select(".x.axis").attr("transform",K.x);g.select(".y2.axis").attr("transform",K.y2);ya&&(y.select(".x.brush").selectAll("rect").attr("height",ja),y.attr("transform",K.context),y.select(".x.axis").attr("transform",K.subx));Aa&&(za.attr("transform",K.legend),eb(k.data.targets,{withTransition:!1}));B({withTransition:!1,withUpdateXDomain:!0})};return k}})(window);
