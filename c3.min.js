...
"path").attr("class","tick major").style("opacity",1E-6),ja=d3.transition(q.exit()).style("opacity",1E-6).remove(),ka=d3.transition(q).style("opacity",1),B,S=G.rangeExtent?G.rangeExtent():Oa(G.range()),h=h.selectAll(".domain").data([0]),h=(h.enter().append("path").attr("class","domain"),d3.transition(h)),K=G.copy(),bb=this.__chart__||K;this.__chart__=K;A.append("line");A.append("text");var la=A.select("line"),ua=ka.select("line"),ba=q.select("text"),va=A.select("text"),wa=ka.select("text");R=(K(1)-
